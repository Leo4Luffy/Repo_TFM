...
<!--
11. Se generan los valores genotípicos y fenotipos para todos los individuos en cada uno de los pedigríes, a partir del uso del comando gg.Population().
-->
...
print('No. of individuals is ' + str(len(Pob_1.inds)))

pickle.dump(Pob_1, open('Pob_1', 'wb'))
...
print('No. of individuals is ' + str(len(Pob_2.inds)))

pickle.dump(Pob_2, open('Pob_2', 'wb'))

###########
# Pedigrí 3
###########

Pedigri_3 = 'Pedigri_3/Pedigri_3.txt' # Se crea el objeto "Pedigri_3", que corresponde al pedigrí 2 generado anteriormente.

Pob_3 = gg.Population(Gen_caracteristicas, pedFile = Pedigri_3, generation = None, qtns = QTN, gfounders = Gen_Pob_base)
print('No. of individuals is ' + str(len(Pob_3.inds)))

pickle.dump(Pob_3, open('Pob_3', 'wb'))

###########
# Pedigrí 4
###########

Pedigri_4 = 'Pedigri_4/Pedigri_4.txt' # Se crea el objeto "Pedigri_4", que corresponde al pedigrí 2 generado anteriormente.

Pob_4 = gg.Population(Gen_caracteristicas, pedFile = Pedigri_4, generation = None, qtns = QTN, gfounders = Gen_Pob_base)
print('No. of individuals is ' + str(len(Pob_4.inds)))

pickle.dump(Pob_4, open('Pob_4', 'wb'))
```
...
<!--
13. Se obtienen los resultados del ssGBLUP para el pedigrí 1.
-->
...
# 2. Se carga el objeto "Gen_Pob_base" ----

Gen_Pob_base = pickle.load(open('Gen_Pob_base', 'rb')) # Se carga el archivo generado en pasos atras "Gen_Pob_base" "Gen_pob_base", que corresponde al genotipo de la población base.

# 3. Se carga el objeto "Pob_1" ----

Pob_1 = pickle.load(open('Pob_1', 'rb')) # Se carga el archivo generado en pasos atras "Pob_1", que corresponde a los valores genotípicos y fenotipos de todos los individuos del pedigrí 1.
...
###########################
# Densidad de SNPs de 1.000
###########################

os.mkdir('Pedigri_1/chip_1') # Se crea la carpeta que contendra todos los resultados al realizar ssGBLUP con una densidad de 1.000 SNPs.

######################################
# Con todos los individuos genotipados
######################################

print('\n' + 'Evaluación mediante ssGBLUP considerando una densidad de 1.000 SNPs y con todos los individuos genotipados.')

os.mkdir('Pedigri_1/chip_1/Geno_1') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con todos los individuos genotipados.

# 4. Se realiza el ssGBLUP ----

ind_gen_1 = np.arange(2091, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 1 al 2091).

mX_1 = gg.do_X([Pob_1.inds[i] for i in ind_gen_1], Gen_caracteristicas, Gen_Pob_base, chips[1], minMaf = 0.05) # Aquí se crea el objeto "mX_1" que corresponde a la matriz de SNPs.

ind_fen = np.arange(1291, dtype = np.int) # Aquí se crea el objeto "ind_fen" que indica que individuos tendrían medida fenotípica (del individuo 1 al 1291, esto es, ningún individuo de la generación F3 tiene fenotipo).

itrait = 0

sel.doEbv(Pob_1, criterion = 'sstep', X = mX_1, yIds = ind_fen, mkrIds = ind_gen_1, h2 = 0.7, nh = Gen_caracteristicas.ploidy)

# 5. Se calculan los valores genéticos aditivos fenotipos (o fenotipos predichos, ebv), los fenotipos observado (y) ----

ebv_ssGBLUP_1 = np.array(list(Pob_1.inds[i].ebv for i in range(Pob_1.n)))
ebv_ssGBLUP_1 = pd.DataFrame(ebv_ssGBLUP_1)
ebv_ssGBLUP_1.to_csv (r'Pedigri_1/chip_1/Geno_1/ebv_1.csv', index = False, header = True)

y_1 = np.array(list(Pob_1.inds[i].y[itrait] for i in range(Pob_1.n)))
y_1 = pd.DataFrame(y_1)
y_1.to_csv (r'Pedigri_1/chip_1/Geno_1/Fenotipo_1.csv', index = False, header = True)

g_add_1 = np.array(list(Pob_1.inds[i].g_add[itrait] for i in range(Pob_1.n)))
g_add_1 = pd.DataFrame(g_add_1)
g_add_1.to_csv (r'Pedigri_1/chip_1/Geno_1/g_add_1.csv', index = False, header = True)

############################################################
# Con los individuos de las generaciones F1 y F2 genotipados
############################################################

print('\n' + 'Evaluación mediante ssGBLUP considerando una densidad de 1.000 SNPs y los individuos de las generaciones F1 y F2 genotipados.')

os.mkdir('Pedigri_1/chip_1/Geno_2') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.

# 4. Se realiza el ssGBLUP ----

ind_gen_2 = np.arange(452, 1292, 1, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 452 al 1291, esto es, los individuos de la generación F1 y F2).

mX_2 = gg.do_X([Pob_1.inds[i] for i in ind_gen_2], Gen_caracteristicas, Gen_Pob_base, chips[1], minMaf = 0.05) # Aquí se crea el objeto "mX_2" que corresponde a la matriz de SNPs.

ind_fen = np.arange(1291, dtype = np.int) # Aquí se crea el objeto "ind_fen" que indica que individuos tendrían medida fenotípica (del individuo 1 al 1291, esto es, ningún individuo de la generación F3 tiene fenotipo).

itrait = 0

sel.doEbv(Pob_1, criterion = 'sstep', X = mX_2, yIds = ind_fen, mkrIds = ind_gen_2, h2 = 0.7, nh = Gen_caracteristicas.ploidy)

# 5. Se calculan los valores genéticos aditivos fenotipos (o fenotipos predichos, ebv), los fenotipos observado (y) ----

ebv_ssGBLUP_2 = np.array(list(Pob_1.inds[i].ebv for i in range(Pob_1.n)))
ebv_ssGBLUP_2 = pd.DataFrame(ebv_ssGBLUP_2)
ebv_ssGBLUP_2.to_csv (r'Pedigri_1/chip_1/Geno_2/ebv_2.csv', index = False, header = True)

y_2 = np.array(list(Pob_1.inds[i].y[itrait] for i in range(Pob_1.n)))
y_2 = pd.DataFrame(y_2)
y_2.to_csv (r'Pedigri_1/chip_1/Geno_2/Fenotipo_2.csv', index = False, header = True)

g_add_2 = np.array(list(Pob_1.inds[i].g_add[itrait] for i in range(Pob_1.n)))
g_add_2 = pd.DataFrame(g_add_2)
g_add_2.to_csv (r'Pedigri_1/chip_1/Geno_2/g_add_2.csv', index = False, header = True)

####################################################
# Con los individuos de la generación F2 genotipados
####################################################

print('\n' + 'Evaluación mediante ssGBLUP considerando una densidad de 1.000 SNPs y los individuos de la generación F2 genotipados.')

os.mkdir('Pedigri_1/chip_1/Geno_3') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.

# 4. Se realiza el ssGBLUP ----

ind_gen_3 = np.arange(492, 1292, 1, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 492 al 1291, esto es, los individuos de la generación F2).

mX_3 = gg.do_X([Pob_1.inds[i] for i in ind_gen_3], Gen_caracteristicas, Gen_Pob_base, chips[1], minMaf = 0.05) # Aquí se crea el objeto "mX_3" que corresponde a la matriz de SNPs.

ind_fen = np.arange(1291, dtype = np.int) # Aquí se crea el objeto "ind_fen" que indica que individuos tendrían medida fenotípica (del individuo 1 al 1291, esto es, ningún individuo de la generación F3 tiene fenotipo).

itrait = 0

sel.doEbv(Pob_1, criterion = 'sstep', X = mX_3, yIds = ind_fen, mkrIds = ind_gen_3, h2 = 0.7, nh = Gen_caracteristicas.ploidy)

# 5. Se calculan los valores genéticos aditivos fenotipos (o fenotipos predichos, ebv), los fenotipos observado (y) ----

ebv_ssGBLUP_3 = np.array(list(Pob_1.inds[i].ebv for i in range(Pob_1.n)))
ebv_ssGBLUP_3 = pd.DataFrame(ebv_ssGBLUP_3)
ebv_ssGBLUP_3.to_csv (r'Pedigri_1/chip_1/Geno_3/ebv_3.csv', index = False, header = True)

y_3 = np.array(list(Pob_1.inds[i].y[itrait] for i in range(Pob_1.n)))
y_3 = pd.DataFrame(y_3)
y_3.to_csv (r'Pedigri_1/chip_1/Geno_3/Fenotipo_3.csv', index = False, header = True)

g_add_3 = np.array(list(Pob_1.inds[i].g_add[itrait] for i in range(Pob_1.n)))
g_add_3 = pd.DataFrame(g_add_3)
g_add_3.to_csv (r'Pedigri_1/chip_1/Geno_3/g_add_3.csv', index = False, header = True)

############################
# Densidad de SNPs de 10.000
############################

os.mkdir('Pedigri_1/chip_2') # Se crea la carpeta que contendra todos los resultados al realizar ssGBLUP con una densidad de 10.000 SNPs.

######################################
# Con todos los individuos genotipados
######################################

print('\n' + 'Evaluación mediante ssGBLUP considerando una densidad de 10.000 SNPs y con todos los individuos genotipados.')

os.mkdir('Pedigri_1/chip_2/Geno_1') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con todos los individuos genotipados.

# 4. Se realiza el ssGBLUP ----

ind_gen_1 = np.arange(2091, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 1 al 2091).

mX_1 = gg.do_X([Pob_1.inds[i] for i in ind_gen_1], Gen_caracteristicas, Gen_Pob_base, chips[2], minMaf = 0.05) # Aquí se crea el objeto "mX_1" que corresponde a la matriz de SNPs.
...
ebv_ssGBLUP_1 = np.array(list(Pob_1.inds[i].ebv for i in range(Pob_1.n)))
ebv_ssGBLUP_1 = pd.DataFrame(ebv_ssGBLUP_1)
ebv_ssGBLUP_1.to_csv (r'Pedigri_1/chip_2/Geno_1/ebv_1.csv', index = False, header = True)

y_1 = np.array(list(Pob_1.inds[i].y[itrait] for i in range(Pob_1.n)))
y_1 = pd.DataFrame(y_1)
y_1.to_csv (r'Pedigri_1/chip_2/Geno_1/Fenotipo_1.csv', index = False, header = True)

g_add_1 = np.array(list(Pob_1.inds[i].g_add[itrait] for i in range(Pob_1.n)))
g_add_1 = pd.DataFrame(g_add_1)
g_add_1.to_csv (r'Pedigri_1/chip_2/Geno_1/g_add_1.csv', index = False, header = True)

############################################################
# Con los individuos de las generaciones F1 y F2 genotipados
############################################################

print('\n' + 'Evaluación mediante ssGBLUP considerando una densidad de 10.000 SNPs y los individuos de las generaciones F1 y F2 genotipados.')

os.mkdir('Pedigri_1/chip_2/Geno_2') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.

# 4. Se realiza el ssGBLUP ----

ind_gen_2 = np.arange(452, 1292, 1, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 452 al 1291, esto es, los individuos de la generación F1 y F2).

mX_2 = gg.do_X([Pob_1.inds[i] for i in ind_gen_2], Gen_caracteristicas, Gen_Pob_base, chips[2], minMaf = 0.05) # Aquí se crea el objeto "mX_2" que corresponde a la matriz de SNPs.
...
ebv_ssGBLUP_2 = np.array(list(Pob_1.inds[i].ebv for i in range(Pob_1.n)))
ebv_ssGBLUP_2 = pd.DataFrame(ebv_ssGBLUP_2)
ebv_ssGBLUP_2.to_csv (r'Pedigri_1/chip_2/Geno_2/ebv_2.csv', index = False, header = True)

y_2 = np.array(list(Pob_1.inds[i].y[itrait] for i in range(Pob_1.n)))
y_2 = pd.DataFrame(y_2)
y_2.to_csv (r'Pedigri_1/chip_2/Geno_2/Fenotipo_2.csv', index = False, header = True)

g_add_2 = np.array(list(Pob_1.inds[i].g_add[itrait] for i in range(Pob_1.n)))
g_add_2 = pd.DataFrame(g_add_2)
g_add_2.to_csv (r'Pedigri_1/chip_2/Geno_2/g_add_2.csv', index = False, header = True)

####################################################
# Con los individuos de la generación F2 genotipados
####################################################

print('\n' + 'Evaluación mediante ssGBLUP considerando una densidad de 10.000 SNPs y los individuos de la generación F2 genotipados.')

os.mkdir('Pedigri_1/chip_2/Geno_3') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.

# 4. Se realiza el ssGBLUP ----

ind_gen_3 = np.arange(492, 1292, 1, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 492 al 1291, esto es, los individuos de la generación F2).

mX_3 = gg.do_X([Pob_1.inds[i] for i in ind_gen_3], Gen_caracteristicas, Gen_Pob_base, chips[2], minMaf = 0.05) # Aquí se crea el objeto "mX_3" que corresponde a la matriz de SNPs.
...
ebv_ssGBLUP_3 = np.array(list(Pob_1.inds[i].ebv for i in range(Pob_1.n)))
ebv_ssGBLUP_3 = pd.DataFrame(ebv_ssGBLUP_3)
ebv_ssGBLUP_3.to_csv (r'Pedigri_1/chip_2/Geno_3/ebv_3.csv', index = False, header = True)

y_3 = np.array(list(Pob_1.inds[i].y[itrait] for i in range(Pob_1.n)))
y_3 = pd.DataFrame(y_3)
y_3.to_csv (r'Pedigri_1/chip_2/Geno_3/Fenotipo_3.csv', index = False, header = True)

g_add_3 = np.array(list(Pob_1.inds[i].g_add[itrait] for i in range(Pob_1.n)))
g_add_3 = pd.DataFrame(g_add_3)
g_add_3.to_csv (r'Pedigri_1/chip_2/Geno_3/g_add_3.csv', index = False, header = True)

#############################
# Densidad de SNPs de 100.000
#############################

os.mkdir('Pedigri_1/chip_3') # Se crea la carpeta que contendra todos los resultados al realizar ssGBLUP con una densidad de 100.000 SNPs.

######################################
# Con todos los individuos genotipados
######################################

print('\n' + 'Evaluación mediante ssGBLUP considerando una densidad de 100.000 SNPs y con todos los individuos genotipados.')

os.mkdir('Pedigri_1/chip_3/Geno_1') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con todos los individuos genotipados.

# 4. Se realiza el ssGBLUP ----

ind_gen_1 = np.arange(2091, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 1 al 2091).

mX_1 = gg.do_X([Pob_1.inds[i] for i in ind_gen_1], Gen_caracteristicas, Gen_Pob_base, chips[3], minMaf = 0.05) # Aquí se crea el objeto "mX_1" que corresponde a la matriz de SNPs.
...
ebv_ssGBLUP_1 = np.array(list(Pob_1.inds[i].ebv for i in range(Pob_1.n)))
ebv_ssGBLUP_1 = pd.DataFrame(ebv_ssGBLUP_1)
ebv_ssGBLUP_1.to_csv (r'Pedigri_1/chip_3/Geno_1/ebv_1.csv', index = False, header = True)

y_1 = np.array(list(Pob_1.inds[i].y[itrait] for i in range(Pob_1.n)))
y_1 = pd.DataFrame(y_1)
y_1.to_csv (r'Pedigri_1/chip_3/Geno_1/Fenotipo_1.csv', index = False, header = True)

g_add_1 = np.array(list(Pob_1.inds[i].g_add[itrait] for i in range(Pob_1.n)))
g_add_1 = pd.DataFrame(g_add_1)
g_add_1.to_csv (r'Pedigri_1/chip_3/Geno_1/g_add_1.csv', index = False, header = True)
...
os.mkdir('Pedigri_1/chip_3/Geno_2') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.

# 4. Se realiza el ssGBLUP ----

ind_gen_2 = np.arange(452, 1292, 1, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 452 al 1291, esto es, los individuos de la generación F1 y F2).

mX_2 = gg.do_X([Pob_1.inds[i] for i in ind_gen_2], Gen_caracteristicas, Gen_Pob_base, chips[3], minMaf = 0.05) # Aquí se crea el objeto "mX_2" que corresponde a la matriz de SNPs.
...
ebv_ssGBLUP_2 = np.array(list(Pob_1.inds[i].ebv for i in range(Pob_1.n)))
ebv_ssGBLUP_2 = pd.DataFrame(ebv_ssGBLUP_2)
ebv_ssGBLUP_2.to_csv (r'Pedigri_1/chip_3/Geno_2/ebv_2.csv', index = False, header = True)

y_2 = np.array(list(Pob_1.inds[i].y[itrait] for i in range(Pob_1.n)))
y_2 = pd.DataFrame(y_2)
y_2.to_csv (r'Pedigri_1/chip_3/Geno_2/Fenotipo_2.csv', index = False, header = True)

g_add_2 = np.array(list(Pob_1.inds[i].g_add[itrait] for i in range(Pob_1.n)))
g_add_2 = pd.DataFrame(g_add_2)
g_add_2.to_csv (r'Pedigri_1/chip_3/Geno_2/g_add_2.csv', index = False, header = True)
...
os.mkdir('Pedigri_1/chip_3/Geno_3') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.

# 4. Se realiza el ssGBLUP ----

ind_gen_3 = np.arange(492, 1292, 1, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 492 al 1291, esto es, los individuos de la generación F2).

mX_3 = gg.do_X([Pob_1.inds[i] for i in ind_gen_3], Gen_caracteristicas, Gen_Pob_base, chips[3], minMaf = 0.05) # Aquí se crea el objeto "mX_3" que corresponde a la matriz de SNPs.
...
ebv_ssGBLUP_3 = np.array(list(Pob_1.inds[i].ebv for i in range(Pob_1.n)))
ebv_ssGBLUP_3 = pd.DataFrame(ebv_ssGBLUP_3)
ebv_ssGBLUP_3.to_csv (r'Pedigri_1/chip_3/Geno_3/ebv_3.csv', index = False, header = True)

y_3 = np.array(list(Pob_1.inds[i].y[itrait] for i in range(Pob_1.n)))
y_3 = pd.DataFrame(y_3)
y_3.to_csv (r'Pedigri_1/chip_3/Geno_3/Fenotipo_3.csv', index = False, header = True)

g_add_3 = np.array(list(Pob_1.inds[i].g_add[itrait] for i in range(Pob_1.n)))
g_add_3 = pd.DataFrame(g_add_3)
g_add_3.to_csv (r'Pedigri_1/chip_3/Geno_3/g_add_3.csv', index = False, header = True)
```

<!--
14. Se obtienen los resultados para el pedigrí 2.
-->
...
Pob_2 = pickle.load(open('Pob_2', 'rb')) # Se carga el archivo generado en pasos atras "Pob_2", que corresponde a... del pedigrí 2.

print('Num. de individuos es ' + str(len(Pob_2.inds))) # Se prueba si carga bien indicando el número de individuos en el pedigrí que es de 2071.

###########################
# Densidad de SNPs de 1.000
###########################

os.mkdir('Pedigri_2/chip_1') # Se crea la carpeta que contendra todos los resultados al realizar ssGBLUP con una densidad de 1.000 SNPs.
...
os.mkdir('Pedigri_2/chip_1/Geno_1') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con todos los individuos genotipados.

# 4. Se

ind_gen_1 = np.arange(2071, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 1 al 2071).

mX_1 = gg.do_X([Pob_2.inds[i] for i in ind_gen_1], Gen_caracteristicas, Gen_Pob_base, chips[1], minMaf = 0.05) # Aquí se crea el objeto "mX_1" que corresponde a la matriz de SNPs.

ind_fen = np.arange(1271, dtype = np.int) # Aquí se crea el objeto "ind_fen" que indica que individuos tendrían medida fenotípica (del individuo 1 al 1271, esto es, ningún individuo de la generación F3 tiene fenotipo).

itrait = 0

sel.doEbv(Pob_2, criterion = 'sstep', X = mX_1, yIds = ind_fen, mkrIds = ind_gen_1, h2 = 0.7, nh = Gen_caracteristicas.ploidy)

# 5. Se

ebv_ssGBLUP_1 = np.array(list(Pob_2.inds[i].ebv for i in range(Pob_2.n)))
ebv_ssGBLUP_1 = pd.DataFrame(ebv_ssGBLUP_1)
ebv_ssGBLUP_1.to_csv (r'Pedigri_2/chip_1/Geno_1/ebv_1.csv', index = False, header = True)

y_1 = np.array(list(Pob_2.inds[i].y[itrait] for i in range(Pob_2.n)))
y_1 = pd.DataFrame(y_1)
y_1.to_csv (r'Pedigri_2/chip_1/Geno_1/Fenotipo_1.csv', index = False, header = True)

g_add_1 = np.array(list(Pob_2.inds[i].g_add[itrait] for i in range(Pob_2.n)))
g_add_1 = pd.DataFrame(g_add_1)
g_add_1.to_csv (r'Pedigri_2/chip_1/Geno_1/g_add_1.csv', index = False, header = True)
...
os.mkdir('Pedigri_2/chip_1/Geno_2') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.

# 4. Se

ind_gen_2 = np.arange(452, 1272, 1, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 452 al 1271, esto es, los individuos de la generación F1 y F2).

mX_2 = gg.do_X([Pob_2.inds[i] for i in ind_gen_2], Gen_caracteristicas, Gen_Pob_base, chips[1], minMaf = 0.05) # Aquí se crea el objeto "mX_2" que corresponde a la matriz de SNPs.

ind_fen = np.arange(1271, dtype = np.int) # Aquí se crea el objeto "ind_fen" que indica que individuos tendrían medida fenotípica (del individuo 1 al 1271, esto es, ningún individuo de la generación F3 tiene fenotipo).

itrait = 0

sel.doEbv(Pob_2, criterion = 'sstep', X = mX_2, yIds = ind_fen, mkrIds = ind_gen_2, h2 = 0.7, nh = Gen_caracteristicas.ploidy)

# 5. Se

ebv_ssGBLUP_2 = np.array(list(Pob_2.inds[i].ebv for i in range(Pob_2.n)))
ebv_ssGBLUP_2 = pd.DataFrame(ebv_ssGBLUP_2)
ebv_ssGBLUP_2.to_csv (r'Pedigri_2/chip_1/Geno_2/ebv_2.csv', index = False, header = True)

y_2 = np.array(list(Pob_2.inds[i].y[itrait] for i in range(Pob_2.n)))
y_2 = pd.DataFrame(y_2)
y_2.to_csv (r'Pedigri_2/chip_1/Geno_2/Fenotipo_2.csv', index = False, header = True)

g_add_2 = np.array(list(Pob_2.inds[i].g_add[itrait] for i in range(Pob_2.n)))
g_add_2 = pd.DataFrame(g_add_2)
g_add_2.to_csv (r'Pedigri_2/chip_1/Geno_2/g_add_2.csv', index = False, header = True)
...
os.mkdir('Pedigri_2/chip_1/Geno_3') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.

# 4. Se

ind_gen_3 = np.arange(492, 1272, 1, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 492 al 1271, esto es, los individuos de la generación F2).

mX_3 = gg.do_X([Pob_2.inds[i] for i in ind_gen_3], Gen_caracteristicas, Gen_Pob_base, chips[1], minMaf = 0.05) # Aquí se crea el objeto "mX_3" que corresponde a la matriz de SNPs.

ind_fen = np.arange(1271, dtype = np.int) # Aquí se crea el objeto "ind_fen" que indica que individuos tendrían medida fenotípica (del individuo 1 al 1271, esto es, ningún individuo de la generación F3 tiene fenotipo).

itrait = 0

sel.doEbv(Pob_2, criterion = 'sstep', X = mX_3, yIds = ind_fen, mkrIds = ind_gen_3, h2 = 0.7, nh = Gen_caracteristicas.ploidy)

# 5. Se

ebv_ssGBLUP_3 = np.array(list(Pob_2.inds[i].ebv for i in range(Pob_2.n)))
ebv_ssGBLUP_3 = pd.DataFrame(ebv_ssGBLUP_3)
ebv_ssGBLUP_3.to_csv (r'Pedigri_2/chip_1/Geno_3/ebv_3.csv', index = False, header = True)

y_3 = np.array(list(Pob_2.inds[i].y[itrait] for i in range(Pob_2.n)))
y_3 = pd.DataFrame(y_3)
y_3.to_csv (r'Pedigri_2/chip_1/Geno_3/Fenotipo_3.csv', index = False, header = True)

g_add_3 = np.array(list(Pob_2.inds[i].g_add[itrait] for i in range(Pob_2.n)))
g_add_3 = pd.DataFrame(g_add_3)
g_add_3.to_csv (r'Pedigri_2/chip_1/Geno_3/g_add_3.csv', index = False, header = True)

############################
# Densidad de SNPs de 10.000
############################

os.mkdir('Pedigri_2/chip_2') # Se crea la carpeta que contendra todos los resultados al realizar ssGBLUP con una densidad de 10.000 SNPs.
...
os.mkdir('Pedigri_2/chip_2/Geno_1') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con todos los individuos genotipados.

# 4. Se

ind_gen_1 = np.arange(2071, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 1 al 2071).

mX_1 = gg.do_X([Pob_2.inds[i] for i in ind_gen_1], Gen_caracteristicas, Gen_Pob_base, chips[2], minMaf = 0.05) # Aquí se crea el objeto "mX_1" que corresponde a la matriz de SNPs.
...
ebv_ssGBLUP_1 = np.array(list(Pob_2.inds[i].ebv for i in range(Pob_2.n)))
ebv_ssGBLUP_1 = pd.DataFrame(ebv_ssGBLUP_1)
ebv_ssGBLUP_1.to_csv (r'Pedigri_2/chip_2/Geno_1/ebv_1.csv', index = False, header = True)

y_1 = np.array(list(Pob_2.inds[i].y[itrait] for i in range(Pob_2.n)))
y_1 = pd.DataFrame(y_1)
y_1.to_csv (r'Pedigri_2/chip_2/Geno_1/Fenotipo_1.csv', index = False, header = True)

g_add_1 = np.array(list(Pob_2.inds[i].g_add[itrait] for i in range(Pob_2.n)))
g_add_1 = pd.DataFrame(g_add_1)
g_add_1.to_csv (r'Pedigri_2/chip_2/Geno_1/g_add_1.csv', index = False, header = True)
...
os.mkdir('Pedigri_2/chip_2/Geno_2') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.

# 4. Se

ind_gen_2 = np.arange(452, 1272, 1, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 452 al 1271, esto es, los individuos de la generación F1 y F2).

mX_2 = gg.do_X([Pob_2.inds[i] for i in ind_gen_2], Gen_caracteristicas, Gen_Pob_base, chips[2], minMaf = 0.05) # Aquí se crea el objeto "mX_2" que corresponde a la matriz de SNPs.
...
ebv_ssGBLUP_2 = np.array(list(Pob_2.inds[i].ebv for i in range(Pob_2.n)))
ebv_ssGBLUP_2 = pd.DataFrame(ebv_ssGBLUP_2)
ebv_ssGBLUP_2.to_csv (r'Pedigri_2/chip_2/Geno_2/ebv_2.csv', index = False, header = True)

y_2 = np.array(list(Pob_2.inds[i].y[itrait] for i in range(Pob_2.n)))
y_2 = pd.DataFrame(y_2)
y_2.to_csv (r'Pedigri_2/chip_2/Geno_2/Fenotipo_2.csv', index = False, header = True)

g_add_2 = np.array(list(Pob_2.inds[i].g_add[itrait] for i in range(Pob_2.n)))
g_add_2 = pd.DataFrame(g_add_2)
g_add_2.to_csv (r'Pedigri_2/chip_2/Geno_2/g_add_2.csv', index = False, header = True)
...
os.mkdir('Pedigri_2/chip_2/Geno_3') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.

# 4. Se

ind_gen_3 = np.arange(492, 1272, 1, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 492 al 1271, esto es, los individuos de la generación F2).

mX_3 = gg.do_X([Pob_2.inds[i] for i in ind_gen_3], Gen_caracteristicas, Gen_Pob_base, chips[2], minMaf = 0.05) # Aquí se crea el objeto "mX_3" que corresponde a la matriz de SNPs.

ind_fen = np.arange(1271, dtype = np.int) # Aquí se crea el objeto "ind_fen" que indica que individuos tendrían medida fenotípica (del individuo 1 al 1291, esto es, ningún individuo de la generación F3 tiene fenotipo).
...
ebv_ssGBLUP_3 = np.array(list(Pob_2.inds[i].ebv for i in range(Pob_2.n)))
ebv_ssGBLUP_3 = pd.DataFrame(ebv_ssGBLUP_3)
ebv_ssGBLUP_3.to_csv (r'Pedigri_2/chip_2/Geno_3/ebv_3.csv', index = False, header = True)

y_3 = np.array(list(Pob_2.inds[i].y[itrait] for i in range(Pob_2.n)))
y_3 = pd.DataFrame(y_3)
y_3.to_csv (r'Pedigri_2/chip_2/Geno_3/Fenotipo_3.csv', index = False, header = True)

g_add_3 = np.array(list(Pob_2.inds[i].g_add[itrait] for i in range(Pob_2.n)))
g_add_3 = pd.DataFrame(g_add_3)
g_add_3.to_csv (r'Pedigri_2/chip_2/Geno_3/g_add_3.csv', index = False, header = True)

#############################
# Densidad de SNPs de 100.000
#############################

os.mkdir('Pedigri_2/chip_3') # Se crea la carpeta que contendra todos los resultados al realizar ssGBLUP con una densidad de 100.000 SNPs.
...
os.mkdir('Pedigri_2/chip_3/Geno_1') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con todos los individuos genotipados.

# 4. Se

ind_gen_1 = np.arange(2071, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 1 al 2071).

mX_1 = gg.do_X([Pob_2.inds[i] for i in ind_gen_1], Gen_caracteristicas, Gen_Pob_base, chips[3], minMaf = 0.05) # Aquí se crea el objeto "mX_1" que corresponde a la matriz de SNPs.
...
ebv_ssGBLUP_1 = np.array(list(Pob_2.inds[i].ebv for i in range(Pob_2.n)))
ebv_ssGBLUP_1 = pd.DataFrame(ebv_ssGBLUP_1)
ebv_ssGBLUP_1.to_csv (r'Pedigri_2/chip_3/Geno_1/ebv_1.csv', index = False, header = True)

y_1 = np.array(list(Pob_2.inds[i].y[itrait] for i in range(Pob_2.n)))
y_1 = pd.DataFrame(y_1)
y_1.to_csv (r'Pedigri_2/chip_3/Geno_1/Fenotipo_1.csv', index = False, header = True)

g_add_1 = np.array(list(Pob_2.inds[i].g_add[itrait] for i in range(Pob_2.n)))
g_add_1 = pd.DataFrame(g_add_1)
g_add_1.to_csv (r'Pedigri_2/chip_3/Geno_1/g_add_1.csv', index = False, header = True)
...
os.mkdir('Pedigri_2/chip_3/Geno_2') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.

# 4. Se

ind_gen_2 = np.arange(452, 1272, 1, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 452 al 1271, esto es, los individuos de la generación F1 y F2).

mX_2 = gg.do_X([Pob_2.inds[i] for i in ind_gen_2], Gen_caracteristicas, Gen_Pob_base, chips[3], minMaf = 0.05) # Aquí se crea el objeto "mX_2" que corresponde a la matriz de SNPs.
...
ebv_ssGBLUP_2 = np.array(list(Pob_2.inds[i].ebv for i in range(Pob_2.n)))
ebv_ssGBLUP_2 = pd.DataFrame(ebv_ssGBLUP_2)
ebv_ssGBLUP_2.to_csv (r'Pedigri_2/chip_3/Geno_2/ebv_2.csv', index = False, header = True)

y_2 = np.array(list(Pob_2.inds[i].y[itrait] for i in range(Pob_2.n)))
y_2 = pd.DataFrame(y_2)
y_2.to_csv (r'Pedigri_2/chip_3/Geno_2/Fenotipo_2.csv', index = False, header = True)

g_add_2 = np.array(list(Pob_2.inds[i].g_add[itrait] for i in range(Pob_2.n)))
g_add_2 = pd.DataFrame(g_add_2)
g_add_2.to_csv (r'Pedigri_2/chip_3/Geno_2/g_add_2.csv', index = False, header = True)
...
os.mkdir('Pedigri_2/chip_3/Geno_3') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.

# 4. Se

ind_gen_3 = np.arange(492, 1272, 1, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 492 al 1271, esto es, los individuos de la generación F2).

mX_3 = gg.do_X([Pob_2.inds[i] for i in ind_gen_3], Gen_caracteristicas, Gen_Pob_base, chips[3], minMaf = 0.05) # Aquí se crea el objeto "mX_3" que corresponde a la matriz de SNPs.
...
ebv_ssGBLUP_3 = np.array(list(Pob_2.inds[i].ebv for i in range(Pob_2.n)))
ebv_ssGBLUP_3 = pd.DataFrame(ebv_ssGBLUP_3)
ebv_ssGBLUP_3.to_csv (r'Pedigri_2/chip_3/Geno_3/ebv_3.csv', index = False, header = True)

y_3 = np.array(list(Pob_2.inds[i].y[itrait] for i in range(Pob_2.n)))
y_3 = pd.DataFrame(y_3)
y_3.to_csv (r'Pedigri_2/chip_3/Geno_3/Fenotipo_3.csv', index = False, header = True)

g_add_3 = np.array(list(Pob_2.inds[i].g_add[itrait] for i in range(Pob_2.n)))
g_add_3 = pd.DataFrame(g_add_3)
g_add_3.to_csv (r'Pedigri_2/chip_3/Geno_3/g_add_3.csv', index = False, header = True)
```

<!--
15. Se obtienen los resultados para el pedigrí 3.
-->
...
Pob_3 = pickle.load(open('Pob_3', 'rb')) # Se carga el archivo generado en pasos atras "Pob_3", que corresponde a... del pedigrí 3.

print('Num. de individuos es ' + str(len(Pob_3.inds))) # Se prueba si carga bien indicando el número de individuos en el pedigrí que es de 2061.

###########################
# Densidad de SNPs de 1.000
###########################

os.mkdir('Pedigri_3/chip_1') # Se crea la carpeta que contendra todos los resultados al realizar ssGBLUP con una densidad de 1.000 SNPs.
...
os.mkdir('Pedigri_3/chip_1/Geno_1') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con todos los individuos genotipados.

# 4. Se

ind_gen_1 = np.arange(2061, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 1 al 2061).

mX_1 = gg.do_X([Pob_3.inds[i] for i in ind_gen_1], Gen_caracteristicas, Gen_Pob_base, chips[1], minMaf = 0.05) # Aquí se crea el objeto "mX_1" que corresponde a la matriz de SNPs.

ind_fen = np.arange(1261, dtype = np.int) # Aquí se crea el objeto "ind_fen" que indica que individuos tendrían medida fenotípica (del individuo 1 al 1261, esto es, ningún individuo de la generación F3 tiene fenotipo).

itrait = 0

sel.doEbv(Pob_3, criterion = 'sstep', X = mX_1, yIds = ind_fen, mkrIds = ind_gen_1, h2 = 0.7, nh = Gen_caracteristicas.ploidy)

# 5. Se

ebv_ssGBLUP_1 = np.array(list(Pob_3.inds[i].ebv for i in range(Pob_3.n)))
ebv_ssGBLUP_1 = pd.DataFrame(ebv_ssGBLUP_1)
ebv_ssGBLUP_1.to_csv (r'Pedigri_3/chip_1/Geno_1/ebv_1.csv', index = False, header = True)

y_1 = np.array(list(Pob_3.inds[i].y[itrait] for i in range(Pob_3.n)))
y_1 = pd.DataFrame(y_1)
y_1.to_csv (r'Pedigri_3/chip_1/Geno_1/Fenotipo_1.csv', index = False, header = True)

g_add_1 = np.array(list(Pob_3.inds[i].g_add[itrait] for i in range(Pob_3.n)))
g_add_1 = pd.DataFrame(g_add_1)
g_add_1.to_csv (r'Pedigri_3/chip_1/Geno_1/g_add_1.csv', index = False, header = True)
...
os.mkdir('Pedigri_3/chip_1/Geno_2') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.

# 4. Se

ind_gen_2 = np.arange(452, 1262, 1, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 452 al 1261, esto es, los individuos de la generación F1 y F2).

mX_2 = gg.do_X([Pob_3.inds[i] for i in ind_gen_2], Gen_caracteristicas, Gen_Pob_base, chips[1], minMaf = 0.05) # Aquí se crea el objeto "mX_2" que corresponde a la matriz de SNPs.

ind_fen = np.arange(1261, dtype = np.int) # Aquí se crea el objeto "ind_fen" que indica que individuos tendrían medida fenotípica (del individuo 1 al 1261, esto es, ningún individuo de la generación F3 tiene fenotipo).

itrait = 0

sel.doEbv(Pob_3, criterion = 'sstep', X = mX_2, yIds = ind_fen, mkrIds = ind_gen_2, h2 = 0.7, nh = Gen_caracteristicas.ploidy)

# 5. Se

ebv_ssGBLUP_2 = np.array(list(Pob_3.inds[i].ebv for i in range(Pob_3.n)))
ebv_ssGBLUP_2 = pd.DataFrame(ebv_ssGBLUP_2)
ebv_ssGBLUP_2.to_csv (r'Pedigri_3/chip_1/Geno_2/ebv_2.csv', index = False, header = True)

y_2 = np.array(list(Pob_3.inds[i].y[itrait] for i in range(Pob_3.n)))
y_2 = pd.DataFrame(y_2)
y_2.to_csv (r'Pedigri_3/chip_1/Geno_2/Fenotipo_2.csv', index = False, header = True)

g_add_2 = np.array(list(Pob_3.inds[i].g_add[itrait] for i in range(Pob_3.n)))
g_add_2 = pd.DataFrame(g_add_2)
g_add_2.to_csv (r'Pedigri_3/chip_1/Geno_2/g_add_2.csv', index = False, header = True)
...
os.mkdir('Pedigri_3/chip_1/Geno_3') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.

# 4. Se

ind_gen_3 = np.arange(492, 1262, 1, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 492 al 1261, esto es, los individuos de la generación F2).

mX_3 = gg.do_X([Pob_3.inds[i] for i in ind_gen_3], Gen_caracteristicas, Gen_Pob_base, chips[1], minMaf = 0.05) # Aquí se crea el objeto "mX_3" que corresponde a la matriz de SNPs.

ind_fen = np.arange(1261, dtype = np.int) # Aquí se crea el objeto "ind_fen" que indica que individuos tendrían medida fenotípica (del individuo 1 al 1261, esto es, ningún individuo de la generación F3 tiene fenotipo).

itrait = 0

sel.doEbv(Pob_3, criterion = 'sstep', X = mX_3, yIds = ind_fen, mkrIds = ind_gen_3, h2 = 0.7, nh = Gen_caracteristicas.ploidy)

# 5. Se

ebv_ssGBLUP_3 = np.array(list(Pob_3.inds[i].ebv for i in range(Pob_3.n)))
ebv_ssGBLUP_3 = pd.DataFrame(ebv_ssGBLUP_3)
ebv_ssGBLUP_3.to_csv (r'Pedigri_3/chip_1/Geno_3/ebv_3.csv', index = False, header = True)

y_3 = np.array(list(Pob_3.inds[i].y[itrait] for i in range(Pob_3.n)))
y_3 = pd.DataFrame(y_3)
y_3.to_csv (r'Pedigri_3/chip_1/Geno_3/Fenotipo_3.csv', index = False, header = True)

g_add_3 = np.array(list(Pob_3.inds[i].g_add[itrait] for i in range(Pob_3.n)))
g_add_3 = pd.DataFrame(g_add_3)
g_add_3.to_csv (r'Pedigri_3/chip_1/Geno_3/g_add_3.csv', index = False, header = True)

############################
# Densidad de SNPs de 10.000
############################

os.mkdir('Pedigri_3/chip_2') # Se crea la carpeta que contendra todos los resultados al realizar ssGBLUP con una densidad de 10.000 SNPs.
...
os.mkdir('Pedigri_3/chip_2/Geno_1') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con todos los individuos genotipados.

# 4. Se

ind_gen_1 = np.arange(2061, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 1 al 2061).

mX_1 = gg.do_X([Pob_3.inds[i] for i in ind_gen_1], Gen_caracteristicas, Gen_Pob_base, chips[2], minMaf = 0.05) # Aquí se crea el objeto "mX_1" que corresponde a la matriz de SNPs.
...
ebv_ssGBLUP_1 = np.array(list(Pob_3.inds[i].ebv for i in range(Pob_3.n)))
ebv_ssGBLUP_1 = pd.DataFrame(ebv_ssGBLUP_1)
ebv_ssGBLUP_1.to_csv (r'Pedigri_3/chip_2/Geno_1/ebv_1.csv', index = False, header = True)

y_1 = np.array(list(Pob_3.inds[i].y[itrait] for i in range(Pob_3.n)))
y_1 = pd.DataFrame(y_1)
y_1.to_csv (r'Pedigri_3/chip_2/Geno_1/Fenotipo_1.csv', index = False, header = True)

g_add_1 = np.array(list(Pob_3.inds[i].g_add[itrait] for i in range(Pob_3.n)))
g_add_1 = pd.DataFrame(g_add_1)
g_add_1.to_csv (r'Pedigri_3/chip_2/Geno_1/g_add_1.csv', index = False, header = True)
...
os.mkdir('Pedigri_3/chip_2/Geno_2') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.

# 4. Se

ind_gen_2 = np.arange(452, 1262, 1, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 452 al 1261, esto es, los individuos de la generación F1 y F2).

mX_2 = gg.do_X([Pob_3.inds[i] for i in ind_gen_2], Gen_caracteristicas, Gen_Pob_base, chips[2], minMaf = 0.05) # Aquí se crea el objeto "mX_2" que corresponde a la matriz de SNPs.
...
ebv_ssGBLUP_2 = np.array(list(Pob_3.inds[i].ebv for i in range(Pob_3.n)))
ebv_ssGBLUP_2 = pd.DataFrame(ebv_ssGBLUP_2)
ebv_ssGBLUP_2.to_csv (r'Pedigri_3/chip_2/Geno_2/ebv_2.csv', index = False, header = True)

y_2 = np.array(list(Pob_3.inds[i].y[itrait] for i in range(Pob_3.n)))
y_2 = pd.DataFrame(y_2)
y_2.to_csv (r'Pedigri_3/chip_2/Geno_2/Fenotipo_2.csv', index = False, header = True)

g_add_2 = np.array(list(Pob_3.inds[i].g_add[itrait] for i in range(Pob_3.n)))
g_add_2 = pd.DataFrame(g_add_2)
g_add_2.to_csv (r'Pedigri_3/chip_2/Geno_2/g_add_2.csv', index = False, header = True)
...
os.mkdir('Pedigri_3/chip_2/Geno_3') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.

# 4. Se

ind_gen_3 = np.arange(492, 1262, 1, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 492 al 1261, esto es, los individuos de la generación F2).

mX_3 = gg.do_X([Pob_3.inds[i] for i in ind_gen_3], Gen_caracteristicas, Gen_Pob_base, chips[2], minMaf = 0.05) # Aquí se crea el objeto "mX_3" que corresponde a la matriz de SNPs.
...
ebv_ssGBLUP_3 = np.array(list(Pob_3.inds[i].ebv for i in range(Pob_3.n)))
ebv_ssGBLUP_3 = pd.DataFrame(ebv_ssGBLUP_3)
ebv_ssGBLUP_3.to_csv (r'Pedigri_3/chip_2/Geno_3/ebv_3.csv', index = False, header = True)

y_3 = np.array(list(Pob_3.inds[i].y[itrait] for i in range(Pob_3.n)))
y_3 = pd.DataFrame(y_3)
y_3.to_csv (r'Pedigri_3/chip_2/Geno_3/Fenotipo_3.csv', index = False, header = True)

g_add_3 = np.array(list(Pob_3.inds[i].g_add[itrait] for i in range(Pob_3.n)))
g_add_3 = pd.DataFrame(g_add_3)
g_add_3.to_csv (r'Pedigri_3/chip_2/Geno_3/g_add_3.csv', index = False, header = True)

#############################
# Densidad de SNPs de 100.000
#############################

os.mkdir('Pedigri_3/chip_3') # Se crea la carpeta que contendra todos los resultados al realizar ssGBLUP con una densidad de 100.000 SNPs.
...
os.mkdir('Pedigri_3/chip_3/Geno_1') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con todos los individuos genotipados.

# 4. Se

ind_gen_1 = np.arange(2061, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 1 al 2061).

mX_1 = gg.do_X([Pob_3.inds[i] for i in ind_gen_1], Gen_caracteristicas, Gen_Pob_base, chips[3], minMaf = 0.05) # Aquí se crea el objeto "mX_1" que corresponde a la matriz de SNPs.
...
ebv_ssGBLUP_1 = np.array(list(Pob_3.inds[i].ebv for i in range(Pob_3.n)))
ebv_ssGBLUP_1 = pd.DataFrame(ebv_ssGBLUP_1)
ebv_ssGBLUP_1.to_csv (r'Pedigri_3/chip_3/Geno_1/ebv_1.csv', index = False, header = True)

y_1 = np.array(list(Pob_3.inds[i].y[itrait] for i in range(Pob_3.n)))
y_1 = pd.DataFrame(y_1)
y_1.to_csv (r'Pedigri_3/chip_3/Geno_1/Fenotipo_1.csv', index = False, header = True)

g_add_1 = np.array(list(Pob_3.inds[i].g_add[itrait] for i in range(Pob_3.n)))
g_add_1 = pd.DataFrame(g_add_1)
g_add_1.to_csv (r'Pedigri_3/chip_3/Geno_1/g_add_1.csv', index = False, header = True)
...
os.mkdir('Pedigri_3/chip_3/Geno_2') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.

# 4. Se

ind_gen_2 = np.arange(452, 1262, 1, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 452 al 1261, esto es, los individuos de la generación F1 y F2).

mX_2 = gg.do_X([Pob_3.inds[i] for i in ind_gen_2], Gen_caracteristicas, Gen_Pob_base, chips[3], minMaf = 0.05) # Aquí se crea el objeto "mX_2" que corresponde a la matriz de SNPs.
...
ebv_ssGBLUP_2 = np.array(list(Pob_3.inds[i].ebv for i in range(Pob_3.n)))
ebv_ssGBLUP_2 = pd.DataFrame(ebv_ssGBLUP_2)
ebv_ssGBLUP_2.to_csv (r'Pedigri_3/chip_3/Geno_2/ebv_2.csv', index = False, header = True)

y_2 = np.array(list(Pob_3.inds[i].y[itrait] for i in range(Pob_3.n)))
y_2 = pd.DataFrame(y_2)
y_2.to_csv (r'Pedigri_3/chip_3/Geno_2/Fenotipo_2.csv', index = False, header = True)

g_add_2 = np.array(list(Pob_3.inds[i].g_add[itrait] for i in range(Pob_3.n)))
g_add_2 = pd.DataFrame(g_add_2)
g_add_2.to_csv (r'Pedigri_3/chip_3/Geno_2/g_add_2.csv', index = False, header = True)
...
os.mkdir('Pedigri_3/chip_3/Geno_3') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.

# 4. Se

ind_gen_3 = np.arange(492, 1262, 1, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 492 al 1261, esto es, los individuos de la generación F2).

mX_3 = gg.do_X([Pob_3.inds[i] for i in ind_gen_3], Gen_caracteristicas, Gen_Pob_base, chips[3], minMaf = 0.05) # Aquí se crea el objeto "mX_3" que corresponde a la matriz de SNPs.

ind_fen = np.arange(1261, dtype = np.int) # Aquí se crea el objeto "ind_fen" que indica que individuos tendrían medida fenotípica (del individuo 1 al 1291, esto es, ningún individuo de la generación F3 tiene fenotipo).
...
ebv_ssGBLUP_3 = np.array(list(Pob_3.inds[i].ebv for i in range(Pob_3.n)))
ebv_ssGBLUP_3 = pd.DataFrame(ebv_ssGBLUP_3)
ebv_ssGBLUP_3.to_csv (r'Pedigri_3/chip_3/Geno_3/ebv_3.csv', index = False, header = True)

y_3 = np.array(list(Pob_3.inds[i].y[itrait] for i in range(Pob_3.n)))
y_3 = pd.DataFrame(y_3)
y_3.to_csv (r'Pedigri_3/chip_3/Geno_3/Fenotipo_3.csv', index = False, header = True)

g_add_3 = np.array(list(Pob_3.inds[i].g_add[itrait] for i in range(Pob_3.n)))
g_add_3 = pd.DataFrame(g_add_3)
g_add_3.to_csv (r'Pedigri_3/chip_3/Geno_3/g_add_3.csv', index = False, header = True)
```

<!--
16. Se obtienen los resultados para el pedigrí 4.
-->
...
Pob_4 = pickle.load(open('Pob_4', 'rb')) # Se carga el archivo generado en pasos atras "Pob_3", que corresponde a... del pedigrí 4.

print('Num. de individuos es ' + str(len(Pob_4.inds))) # Se prueba si carga bien indicando el número de individuos en el pedigrí que es de 2131.

###########################
# Densidad de SNPs de 1.000
###########################

os.mkdir('Pedigri_4/chip_1') # Se crea la carpeta que contendra todos los resultados al realizar ssGBLUP con una densidad de 1.000 SNPs.
...
os.mkdir('Pedigri_4/chip_1/Geno_1') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con todos los individuos genotipados.

# 4. Se

ind_gen_1 = np.arange(2131, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 1 al 2131).

mX_1 = gg.do_X([Pob_4.inds[i] for i in ind_gen_1], Gen_caracteristicas, Gen_Pob_base, chips[1], minMaf = 0.05) # Aquí se crea el objeto "mX_1" que corresponde a la matriz de SNPs.

ind_fen = np.arange(1331, dtype = np.int) # Aquí se crea el objeto "ind_fen" que indica que individuos tendrían medida fenotípica (del individuo 1 al 1331, esto es, ningún individuo de la generación F3 tiene fenotipo).

itrait = 0

sel.doEbv(Pob_4, criterion = 'sstep', X = mX_1, yIds = ind_fen, mkrIds = ind_gen_1, h2 = 0.7, nh = Gen_caracteristicas.ploidy)

# 5. Se

ebv_ssGBLUP_1 = np.array(list(Pob_4.inds[i].ebv for i in range(Pob_4.n)))
ebv_ssGBLUP_1 = pd.DataFrame(ebv_ssGBLUP_1)
ebv_ssGBLUP_1.to_csv (r'Pedigri_4/chip_1/Geno_1/ebv_1.csv', index = False, header = True)

y_1 = np.array(list(Pob_4.inds[i].y[itrait] for i in range(Pob_4.n)))
y_1 = pd.DataFrame(y_1)
y_1.to_csv (r'Pedigri_4/chip_1/Geno_1/Fenotipo_1.csv', index = False, header = True)

g_add_1 = np.array(list(Pob_4.inds[i].g_add[itrait] for i in range(Pob_4.n)))
g_add_1 = pd.DataFrame(g_add_1)
g_add_1.to_csv (r'Pedigri_4/chip_1/Geno_1/g_add_1.csv', index = False, header = True)
...
os.mkdir('Pedigri_4/chip_1/Geno_2') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.

# 4. Se

ind_gen_2 = np.arange(452, 1332, 1, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 452 al 1331, esto es, los individuos de la generación F1 y F2).

mX_2 = gg.do_X([Pob_4.inds[i] for i in ind_gen_2], Gen_caracteristicas, Gen_Pob_base, chips[1], minMaf = 0.05) # Aquí se crea el objeto "mX_2" que corresponde a la matriz de SNPs.

ind_fen = np.arange(1331, dtype = np.int) # Aquí se crea el objeto "ind_fen" que indica que individuos tendrían medida fenotípica (del individuo 1 al 1331, esto es, ningún individuo de la generación F3 tiene fenotipo).

itrait = 0

sel.doEbv(Pob_4, criterion = 'sstep', X = mX_2, yIds = ind_fen, mkrIds = ind_gen_2, h2 = 0.7, nh = Gen_caracteristicas.ploidy)

# 5. Se

ebv_ssGBLUP_2 = np.array(list(Pob_4.inds[i].ebv for i in range(Pob_4.n)))
ebv_ssGBLUP_2 = pd.DataFrame(ebv_ssGBLUP_2)
ebv_ssGBLUP_2.to_csv (r'Pedigri_4/chip_1/Geno_2/ebv_2.csv', index = False, header = True)

y_2 = np.array(list(Pob_4.inds[i].y[itrait] for i in range(Pob_4.n)))
y_2 = pd.DataFrame(y_2)
y_2.to_csv (r'Pedigri_4/chip_1/Geno_2/Fenotipo_2.csv', index = False, header = True)

g_add_2 = np.array(list(Pob_4.inds[i].g_add[itrait] for i in range(Pob_4.n)))
g_add_2 = pd.DataFrame(g_add_2)
g_add_2.to_csv (r'Pedigri_4/chip_1/Geno_2/g_add_2.csv', index = False, header = True)
...
os.mkdir('Pedigri_4/chip_1/Geno_3') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.

# 4. Se

ind_gen_3 = np.arange(532, 1332, 1, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 531 al 1331, esto es, los individuos de la generación F2).

mX_3 = gg.do_X([Pob_4.inds[i] for i in ind_gen_3], Gen_caracteristicas, Gen_Pob_base, chips[1], minMaf = 0.05) # Aquí se crea el objeto "mX_3" que corresponde a la matriz de SNPs.

ind_fen = np.arange(1331, dtype = np.int) # Aquí se crea el objeto "ind_fen" que indica que individuos tendrían medida fenotípica (del individuo 1 al 1331, esto es, ningún individuo de la generación F3 tiene fenotipo).

itrait = 0

sel.doEbv(Pob_4, criterion = 'sstep', X = mX_3, yIds = ind_fen, mkrIds = ind_gen_3, h2 = 0.7, nh = Gen_caracteristicas.ploidy)

# 5. Se

ebv_ssGBLUP_3 = np.array(list(Pob_4.inds[i].ebv for i in range(Pob_4.n)))
ebv_ssGBLUP_3 = pd.DataFrame(ebv_ssGBLUP_3)
ebv_ssGBLUP_3.to_csv (r'Pedigri_4/chip_1/Geno_3/ebv_3.csv', index = False, header = True)

y_3 = np.array(list(Pob_4.inds[i].y[itrait] for i in range(Pob_4.n)))
y_3 = pd.DataFrame(y_3)
y_3.to_csv (r'Pedigri_4/chip_1/Geno_3/Fenotipo_3.csv', index = False, header = True)

g_add_3 = np.array(list(Pob_4.inds[i].g_add[itrait] for i in range(Pob_4.n)))
g_add_3 = pd.DataFrame(g_add_3)
g_add_3.to_csv (r'Pedigri_4/chip_1/Geno_3/g_add_3.csv', index = False, header = True)

############################
# Densidad de SNPs de 10.000
############################

os.mkdir('Pedigri_4/chip_2') # Se crea la carpeta que contendra todos los resultados al realizar ssGBLUP con una densidad de 10.000 SNPs.
...
os.mkdir('Pedigri_4/chip_2/Geno_1') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con todos los individuos genotipados.

# 4. Se

ind_gen_1 = np.arange(2131, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 1 al 2131).

mX_1 = gg.do_X([Pob_4.inds[i] for i in ind_gen_1], Gen_caracteristicas, Gen_Pob_base, chips[2], minMaf = 0.05) # Aquí se crea el objeto "mX_1" que corresponde a la matriz de SNPs.
...
ebv_ssGBLUP_1 = np.array(list(Pob_4.inds[i].ebv for i in range(Pob_4.n)))
ebv_ssGBLUP_1 = pd.DataFrame(ebv_ssGBLUP_1)
ebv_ssGBLUP_1.to_csv (r'Pedigri_4/chip_2/Geno_1/ebv_1.csv', index = False, header = True)

y_1 = np.array(list(Pob_4.inds[i].y[itrait] for i in range(Pob_4.n)))
y_1 = pd.DataFrame(y_1)
y_1.to_csv (r'Pedigri_4/chip_2/Geno_1/Fenotipo_1.csv', index = False, header = True)

g_add_1 = np.array(list(Pob_4.inds[i].g_add[itrait] for i in range(Pob_4.n)))
g_add_1 = pd.DataFrame(g_add_1)
g_add_1.to_csv (r'Pedigri_4/chip_2/Geno_1/g_add_1.csv', index = False, header = True)
...
os.mkdir('Pedigri_4/chip_2/Geno_2') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.

# 4. Se

ind_gen_2 = np.arange(452, 1332, 1, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 452 al 1331, esto es, los individuos de la generación F1 y F2).

mX_2 = gg.do_X([Pob_4.inds[i] for i in ind_gen_2], Gen_caracteristicas, Gen_Pob_base, chips[2], minMaf = 0.05) # Aquí se crea el objeto "mX_2" que corresponde a la matriz de SNPs.
...
ebv_ssGBLUP_2 = np.array(list(Pob_4.inds[i].ebv for i in range(Pob_4.n)))
ebv_ssGBLUP_2 = pd.DataFrame(ebv_ssGBLUP_2)
ebv_ssGBLUP_2.to_csv (r'Pedigri_4/chip_2/Geno_2/ebv_2.csv', index = False, header = True)

y_2 = np.array(list(Pob_4.inds[i].y[itrait] for i in range(Pob_4.n)))
y_2 = pd.DataFrame(y_2)
y_2.to_csv (r'Pedigri_4/chip_2/Geno_2/Fenotipo_2.csv', index = False, header = True)

g_add_2 = np.array(list(Pob_4.inds[i].g_add[itrait] for i in range(Pob_4.n)))
g_add_2 = pd.DataFrame(g_add_2)
g_add_2.to_csv (r'Pedigri_4/chip_2/Geno_2/g_add_2.csv', index = False, header = True)
...
os.mkdir('Pedigri_4/chip_2/Geno_3') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.

# 4. Se

ind_gen_3 = np.arange(532, 1332, 1, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 492 al 1261, esto es, los individuos de la generación F2).

mX_3 = gg.do_X([Pob_4.inds[i] for i in ind_gen_3], Gen_caracteristicas, Gen_Pob_base, chips[2], minMaf = 0.05) # Aquí se crea el objeto "mX_3" que corresponde a la matriz de SNPs.
...
ebv_ssGBLUP_3 = np.array(list(Pob_4.inds[i].ebv for i in range(Pob_4.n)))
ebv_ssGBLUP_3 = pd.DataFrame(ebv_ssGBLUP_3)
ebv_ssGBLUP_3.to_csv (r'Pedigri_4/chip_2/Geno_3/ebv_3.csv', index = False, header = True)

y_3 = np.array(list(Pob_4.inds[i].y[itrait] for i in range(Pob_4.n)))
y_3 = pd.DataFrame(y_3)
y_3.to_csv (r'Pedigri_4/chip_2/Geno_3/Fenotipo_3.csv', index = False, header = True)

g_add_3 = np.array(list(Pob_4.inds[i].g_add[itrait] for i in range(Pob_4.n)))
g_add_3 = pd.DataFrame(g_add_3)
g_add_3.to_csv (r'Pedigri_4/chip_2/Geno_3/g_add_3.csv', index = False, header = True)

#############################
# Densidad de SNPs de 100.000
#############################

os.mkdir('Pedigri_4/chip_3') # Se crea la carpeta que contendra todos los resultados al realizar ssGBLUP con una densidad de 100.000 SNPs.
...
os.mkdir('Pedigri_4/chip_3/Geno_1') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con todos los individuos genotipados.

# 4. Se

ind_gen_1 = np.arange(2131, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 1 al 2061).

mX_1 = gg.do_X([Pob_4.inds[i] for i in ind_gen_1], Gen_caracteristicas, Gen_Pob_base, chips[3], minMaf = 0.05) # Aquí se crea el objeto "mX_1" que corresponde a la matriz de SNPs.
...
ebv_ssGBLUP_1 = np.array(list(Pob_4.inds[i].ebv for i in range(Pob_4.n)))
ebv_ssGBLUP_1 = pd.DataFrame(ebv_ssGBLUP_1)
ebv_ssGBLUP_1.to_csv (r'Pedigri_4/chip_3/Geno_1/ebv_1.csv', index = False, header = True)

y_1 = np.array(list(Pob_4.inds[i].y[itrait] for i in range(Pob_4.n)))
y_1 = pd.DataFrame(y_1)
y_1.to_csv (r'Pedigri_4/chip_3/Geno_1/Fenotipo_1.csv', index = False, header = True)

g_add_1 = np.array(list(Pob_4.inds[i].g_add[itrait] for i in range(Pob_4.n)))
g_add_1 = pd.DataFrame(g_add_1)
g_add_1.to_csv (r'Pedigri_4/chip_3/Geno_1/g_add_1.csv', index = False, header = True)
...
os.mkdir('Pedigri_4/chip_3/Geno_2') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.

# 4. Se

ind_gen_2 = np.arange(452, 1332, 1, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 452 al 1331, esto es, los individuos de la generación F1 y F2).

mX_2 = gg.do_X([Pob_4.inds[i] for i in ind_gen_2], Gen_caracteristicas, Gen_Pob_base, chips[3], minMaf = 0.05) # Aquí se crea el objeto "mX_2" que corresponde a la matriz de SNPs.
...
ebv_ssGBLUP_2 = np.array(list(Pob_4.inds[i].ebv for i in range(Pob_4.n)))
ebv_ssGBLUP_2 = pd.DataFrame(ebv_ssGBLUP_2)
ebv_ssGBLUP_2.to_csv (r'Pedigri_4/chip_3/Geno_2/ebv_2.csv', index = False, header = True)

y_2 = np.array(list(Pob_4.inds[i].y[itrait] for i in range(Pob_4.n)))
y_2 = pd.DataFrame(y_2)
y_2.to_csv (r'Pedigri_4/chip_3/Geno_2/Fenotipo_2.csv', index = False, header = True)

g_add_2 = np.array(list(Pob_4.inds[i].g_add[itrait] for i in range(Pob_4.n)))
g_add_2 = pd.DataFrame(g_add_2)
g_add_2.to_csv (r'Pedigri_4/chip_3/Geno_2/g_add_2.csv', index = False, header = True)
...
os.mkdir('Pedigri_4/chip_3/Geno_3') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.

# 4. Se

ind_gen_3 = np.arange(532, 1332, 1, dtype = np.int) # Aquí se crea el objeto "ind_gen" que indica que individuos estan genotipados (del individuo 492 al 1261, esto es, los individuos de la generación F2).

mX_3 = gg.do_X([Pob_4.inds[i] for i in ind_gen_3], Gen_caracteristicas, Gen_Pob_base, chips[3], minMaf = 0.05) # Aquí se crea el objeto "mX_3" que corresponde a la matriz de SNPs.
...
ebv_ssGBLUP_3 = np.array(list(Pob_4.inds[i].ebv for i in range(Pob_4.n)))
ebv_ssGBLUP_3 = pd.DataFrame(ebv_ssGBLUP_3)
ebv_ssGBLUP_3.to_csv (r'Pedigri_4/chip_3/Geno_3/ebv_3.csv', index = False, header = True)

y_3 = np.array(list(Pob_4.inds[i].y[itrait] for i in range(Pob_4.n)))
y_3 = pd.DataFrame(y_3)
y_3.to_csv (r'Pedigri_4/chip_3/Geno_3/Fenotipo_3.csv', index = False, header = True)

g_add_3 = np.array(list(Pob_4.inds[i].g_add[itrait] for i in range(Pob_4.n)))
g_add_3 = pd.DataFrame(g_add_3)
g_add_3.to_csv (r'Pedigri_4/chip_3/Geno_3/g_add_3.csv', index = False, header = True)
```

<!--
17. Se realiza el aBLUP para los distintos pedigríes.
-->

```{python}

os.chdir(dir_tr) # Se indica aquí el cambio hacia el directorio de trabajo, que es donde estan los archivos para hacer el ablup.

# Para el pedigrí 1 ----

os.mkdir('Pedigri_1/chip_0') # Se crea la carpeta que contendra los resultados al realizar aBLUP.

## 1. Se carga el objeto "Pob_1":

Pob_1 = pickle.load(open('Pob_1', 'rb')) # Se carga el archivo generado en pasos atras "Pob_1", que corresponde a los valores genotípicos y fenotipos de todos los individuos del pedigrí 1.

print('Num. de individuos es ' + str(len(Pob_1.inds))) # Se prueba si carga bien indicando el número de individuos en el pedigrí que es de 2091.

## 2. Se realiza el aBLUP:
...
## 3. Se calculan los valores genéticos aditivos fenotipos, los fenotipos y las varianzas genéticas aditivas:

ebv_aBLUP = np.array(list(Pob_1.inds[i].ebv for i in range(Pob_1.n)))
ebv_aBLUP = pd.DataFrame(ebv_aBLUP)
ebv_aBLUP.to_csv (r'Pedigri_1/chip_0/ebv.csv', index = False, header = True)

y_aBLUP = np.array(list(Pob_1.inds[i].y[itrait] for i in range(Pob_1.n)))
y_aBLUP = pd.DataFrame(y_aBLUP)
y_aBLUP.to_csv (r'Pedigri_1/chip_0/Fenotipo.csv', index = False, header = True)

g_add = np.array(list(Pob_1.inds[i].g_add[itrait] for i in range(Pob_1.n)))
g_add = pd.DataFrame(g_add)
g_add.to_csv (r'Pedigri_1/chip_0/g_add.csv', index = False, header = True)

# Para el pedigrí 2 ----

os.mkdir('Pedigri_2/chip_0') # Se crea la carpeta que contendra los resultados al realizar aBLUP.

## 1. Se carga el objeto "Pob_2":

Pob_2 = pickle.load(open('Pob_2', 'rb')) # Se carga el archivo generado en pasos atras "Pob_2", que corresponde a los valores genotípicos y fenotipos de todos los individuos del pedigrí 2.
...
sel.doEbv(Pob_2, criterion = 'blup', yIds = ind_fen, h2 = 0.7, itrait = itrait)

## 3. Se calculan los valores genéticos aditivos fenotipos, los fenotipos y las varianzas genéticas aditivas:

ebv_aBLUP = np.array(list(Pob_2.inds[i].ebv for i in range(Pob_2.n)))
ebv_aBLUP = pd.DataFrame(ebv_aBLUP)
ebv_aBLUP.to_csv (r'Pedigri_2/chip_0/ebv.csv', index = False, header = True)

y_aBLUP = np.array(list(Pob_2.inds[i].y[itrait] for i in range(Pob_2.n)))
y_aBLUP = pd.DataFrame(y_aBLUP)
y_aBLUP.to_csv (r'Pedigri_2/chip_0/Fenotipo.csv', index = False, header = True)

g_add = np.array(list(Pob_2.inds[i].g_add[itrait] for i in range(Pob_2.n)))
g_add = pd.DataFrame(g_add)
g_add.to_csv (r'Pedigri_2/chip_0/g_add.csv', index = False, header = True)

# Para el pedigrí 3 ----

os.mkdir('Pedigri_3/chip_0') # Se crea la carpeta que contendra los resultados al realizar aBLUP.

## 1. Se carga el objeto "Pob_3":

Pob_3 = pickle.load(open('Pob_3', 'rb')) # Se carga el archivo generado en pasos atras "Pob_3", que corresponde a los valores genotípicos y fenotipos de todos los individuos del pedigrí 3.
...
sel.doEbv(Pob_3, criterion = 'blup', yIds = ind_fen, h2 = 0.7, itrait = itrait)

## 3. Se calculan los valores genéticos aditivos fenotipos, los fenotipos y las varianzas genéticas aditivas:

ebv_aBLUP = np.array(list(Pob_3.inds[i].ebv for i in range(Pob_3.n)))
ebv_aBLUP = pd.DataFrame(ebv_aBLUP)
ebv_aBLUP.to_csv (r'Pedigri_3/chip_0/ebv.csv', index = False, header = True)

y_aBLUP = np.array(list(Pob_3.inds[i].y[itrait] for i in range(Pob_3.n)))
y_aBLUP = pd.DataFrame(y_aBLUP)
y_aBLUP.to_csv (r'Pedigri_3/chip_0/Fenotipo.csv', index = False, header = True)

g_add = np.array(list(Pob_3.inds[i].g_add[itrait] for i in range(Pob_3.n)))
g_add = pd.DataFrame(g_add)
g_add.to_csv (r'Pedigri_3/chip_0/g_add.csv', index = False, header = True)

# Para el pedigrí 4 ----

os.mkdir('Pedigri_4/chip_0') # Se crea la carpeta que contendra los resultados al realizar aBLUP.

## 1. Se carga el objeto "Pob_3":

Pob_4 = pickle.load(open('Pob_4', 'rb')) # Se carga el archivo generado en pasos atras "Pob_4", que corresponde a los valores genotípicos y fenotipos de todos los individuos del pedigrí 4.
...
sel.doEbv(Pob_4, criterion = 'blup', yIds = ind_fen, h2 = 0.7, itrait = itrait)

## 3. Se calculan los valores genéticos aditivos fenotipos, los fenotipos y las varianzas genéticas aditivas:

ebv_aBLUP = np.array(list(Pob_4.inds[i].ebv for i in range(Pob_4.n)))
ebv_aBLUP = pd.DataFrame(ebv_aBLUP)
ebv_aBLUP.to_csv (r'Pedigri_4/chip_0/ebv.csv', index = False, header = True)

y_aBLUP = np.array(list(Pob_4.inds[i].y[itrait] for i in range(Pob_4.n)))
y_aBLUP = pd.DataFrame(y_aBLUP)
y_aBLUP.to_csv (r'Pedigri_4/chip_0/Fenotipo.csv', index = False, header = True)

g_add = np.array(list(Pob_4.inds[i].g_add[itrait] for i in range(Pob_4.n)))
g_add = pd.DataFrame(g_add)
g_add.to_csv (r'Pedigri_4/chip_0/g_add.csv', index = False, header = True)
```

<!--
18. Se analizan todos los resultados al hacer el ssGBLUP y aBLUP para los distintos pedigríes, distintos chips (o densidad de SNPs) y distintos individuos genotipados.
-->

```{r Observación de resultados con el programa SeqBreed}

# Pedigrí 1 ----

## Se calcula la correlación entre los fenotipos observados (y) y predichos (ebv) considerando una densidad de SNPs de 1.000 y todos los individuos genotipados:

Fen_pred_1 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_1', 'chip_1', 'Geno_1', 'ebv_1.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_1 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_1', 'chip_1', 'Geno_1', 'Fenotipo_1.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_1_ped1 <- bind_cols(Fen_pred_1, Fen_obse_1) %>%
  slice(1293:2092) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1 al 1291), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '1.000', Genotipados = 'F0-F1-F2-F3')

## Se calcula la correlación entre los fenotipos observados (y) y predichos (ebv) considerando una densidad de SNPs de 1.000 y los individuos de las generaciones F1 y F2 genotipados:

Fen_pred_2 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_1', 'chip_1', 'Geno_2', 'ebv_2.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_2 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_1', 'chip_1', 'Geno_2', 'Fenotipo_2.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_2_ped1 <- bind_cols(Fen_pred_2, Fen_obse_2) %>%
  slice(1293:2092) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1 al 1291), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '1.000', Genotipados = 'F1-F2')

## Se calcula la correlación entre los fenotipos observados (y) y predichos (ebv) considerando una densidad de SNPs de 1.000 y los individuos de la generación F2 genotipados:

Fen_pred_3 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_1', 'chip_1', 'Geno_3', 'ebv_3.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_3 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_1', 'chip_1', 'Geno_3', 'Fenotipo_3.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_3_ped1 <- bind_cols(Fen_pred_3, Fen_obse_3) %>%
  slice(1293:2092) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1 al 1291), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '1.000', Genotipados = 'F2')

## Se calcula la correlación entre los fenotipos observados (y) y predichos (ebv) considerando una densidad de SNPs de 10.000 y todos los individuos genotipados:

Fen_pred_4 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_1', 'chip_2', 'Geno_1', 'ebv_1.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_4 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_1', 'chip_2', 'Geno_1', 'Fenotipo_1.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_4_ped1 <- bind_cols(Fen_pred_4, Fen_obse_4) %>%
  slice(1293:2092) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1 al 1291), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '10.000', Genotipados = 'F0-F1-F2-F3')

## Se calcula la correlación entre los fenotipos observados (y) y predichos (ebv) considerando una densidad de SNPs de 10.000 y los individuos de la generación F1 y F2 genotipados:

Fen_pred_5 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_1', 'chip_2', 'Geno_2', 'ebv_2.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_5 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_1', 'chip_2', 'Geno_2', 'Fenotipo_2.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_5_ped1 <- bind_cols(Fen_pred_5, Fen_obse_5) %>%
  slice(1293:2092) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1 al 1291), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '10.000', Genotipados = 'F1-F2')

## Se calcula la correlación entre los fenotipos observados (y) y predichos (ebv) considerando una densidad de SNPs de 10.000 y los individuos de la generación F2 genotipados:

Fen_pred_6 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_1', 'chip_2', 'Geno_3', 'ebv_3.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_6 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_1', 'chip_2', 'Geno_3', 'Fenotipo_3.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_6_ped1 <- bind_cols(Fen_pred_6, Fen_obse_6) %>%
  slice(1293:2092) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1 al 1291), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '10.000', Genotipados = 'F2')

## Se calcula la correlación entre los fenotipos observados (y) y predichos (ebv) considerando una densidad de SNPs de 100.000 y todos los individuos genotipados:

Fen_pred_7 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_1', 'chip_3', 'Geno_1', 'ebv_1.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_7 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_1', 'chip_3', 'Geno_1', 'Fenotipo_1.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_7_ped1 <- bind_cols(Fen_pred_7, Fen_obse_7) %>%
  slice(1293:2092) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1 al 1291), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '100.000', Genotipados = 'F0-F1-F2-F3')

## Se calcula la correlación entre los fenotipos observados (y) y predichos (ebv) considerando una densidad de SNPs de 100.000 y los individuos de las generaciones F1 y F2 genotipados:

Fen_pred_8 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_1', 'chip_3', 'Geno_2', 'ebv_2.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_8 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_1', 'chip_3', 'Geno_2', 'Fenotipo_2.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_8_ped1 <- bind_cols(Fen_pred_8, Fen_obse_8) %>%
  slice(1293:2092) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1 al 1291), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '100.000', Genotipados = 'F1-F2')

## Se calcula la correlación entre los fenotipos observados (y) y predichos (ebv) considerando una densidad de SNPs de 100.000 y los individuos de la generaciónes F2 genotipados:

Fen_pred_9 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_1', 'chip_3', 'Geno_3', 'ebv_3.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_9 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_1', 'chip_3', 'Geno_3', 'Fenotipo_3.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_9_ped1 <- bind_cols(Fen_pred_9, Fen_obse_9) %>%
  slice(1293:2092) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1 al 1291), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '100.000', Genotipados = 'F2')

## Se calcula la correlación entre los fenotipos observados (y) y predichos (ebv) considerando ningún individuo genotipado:

Fen_pred_10 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_1', 'chip_0', 'ebv.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_10 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_1', 'chip_0', 'Fenotipo.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_10_ped1 <- bind_cols(Fen_pred_10, Fen_obse_10) %>%
  slice(1293:2092) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1 al 1291), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '0', Genotipados = 'Ninguno')

Cor_ped1 <- bind_rows(Cor_1_ped1, Cor_2_ped1, Cor_3_ped1, Cor_4_ped1, Cor_5_ped1, Cor_6_ped1, Cor_7_ped1, Cor_8_ped1, Cor_9_ped1, Cor_10_ped1) %>%
  group_by(Densidad, Genotipados) %>%
  summarise(Correlación = round(cor(Predicho, Observado), digits =  2)) %>%
  ggplot(data = ., aes(x = Densidad, y = Correlación, fill = Genotipados, colour = Genotipados, label = Correlación)) +
  geom_col(position = position_dodge(width = 0.7), width = 0.6, alpha = 0.4) +
  geom_label(colour = 'black', fill = 'white', size = 4, position = position_dodge2(width = 1.0), vjust=-0.3) +
  scale_y_continuous(limits = c(0.0, 1.0)) +
  labs(y = 'Precisión de la predicción (r)', x = 'Densidad del marcador', title = 'Pedigrí 1') +
...
Fen_pred_1 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_2', 'chip_1', 'Geno_1', 'ebv_1.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_1 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_2', 'chip_1', 'Geno_1', 'Fenotipo_1.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_1_ped2 <- bind_cols(Fen_pred_1, Fen_obse_1) %>%
  slice(1273:2072) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1 al 1271), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '1.000', Genotipados = 'F0-F1-F2-F3')
...
Fen_pred_2 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_2', 'chip_1', 'Geno_2', 'ebv_2.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_2 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_2', 'chip_1', 'Geno_2', 'Fenotipo_2.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_2_ped2 <- bind_cols(Fen_pred_2, Fen_obse_2) %>%
  slice(1273:2072) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1 al 1271), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '1.000', Genotipados = 'F1-F2')
...
Fen_pred_3 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_2', 'chip_1', 'Geno_3', 'ebv_3.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_3 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_2', 'chip_1', 'Geno_3', 'Fenotipo_3.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_3_ped2 <- bind_cols(Fen_pred_3, Fen_obse_3) %>%
  slice(1273:2072) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1 al 1291), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '1.000', Genotipados = 'F2')
...
Fen_pred_4 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_2', 'chip_2', 'Geno_1', 'ebv_1.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_4 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_2', 'chip_2', 'Geno_1', 'Fenotipo_1.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_4_ped2 <- bind_cols(Fen_pred_4, Fen_obse_4) %>%
  slice(1273:2072) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1 al 1291), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '10.000', Genotipados = 'F0-F1-F2-F3')
...
Fen_pred_5 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_2', 'chip_2', 'Geno_2', 'ebv_2.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_5 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_2', 'chip_2', 'Geno_2', 'Fenotipo_2.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_5_ped2 <- bind_cols(Fen_pred_5, Fen_obse_5) %>%
  slice(1273:2072) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1 al 1291), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '10.000', Genotipados = 'F1-F2')
...
Fen_pred_6 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_2', 'chip_2', 'Geno_3', 'ebv_3.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_6 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_2', 'chip_2', 'Geno_3', 'Fenotipo_3.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_6_ped2 <- bind_cols(Fen_pred_6, Fen_obse_6) %>%
  slice(1273:2072) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1 al 1291), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '10.000', Genotipados = 'F2')
...
Fen_pred_7 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_2', 'chip_3', 'Geno_1', 'ebv_1.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_7 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_2', 'chip_3', 'Geno_1', 'Fenotipo_1.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_7_ped2 <- bind_cols(Fen_pred_7, Fen_obse_7) %>%
  slice(1273:2072) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1 al 1291), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '100.000', Genotipados = 'F0-F1-F2-F3')
...
Fen_pred_8 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_2', 'chip_3', 'Geno_2', 'ebv_2.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_8 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_2', 'chip_3', 'Geno_2', 'Fenotipo_2.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_8_ped2 <- bind_cols(Fen_pred_8, Fen_obse_8) %>%
  slice(1273:2072) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1 al 1291), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '100.000', Genotipados = 'F1-F2')
...
Fen_pred_9 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_2', 'chip_3', 'Geno_3', 'ebv_3.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_9 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_2', 'chip_3', 'Geno_3', 'Fenotipo_3.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_9_ped2 <- bind_cols(Fen_pred_9, Fen_obse_9) %>%
  slice(1273:2072) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1 al 1291), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '100.000', Genotipados = 'F2')
...
Fen_pred_10 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_2', 'chip_0', 'ebv.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_10 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_2', 'chip_0', 'Fenotipo.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_10_ped2 <- bind_cols(Fen_pred_10, Fen_obse_10) %>%
  slice(1273:2072) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1 al 1291), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '0', Genotipados = 'Ninguno')

Cor_ped2 <- bind_rows(Cor_1_ped2, Cor_2_ped2, Cor_3_ped2, Cor_4_ped2, Cor_5_ped2, Cor_6_ped2, Cor_7_ped2, Cor_8_ped2, Cor_9_ped2, Cor_10_ped2) %>%
  group_by(Densidad, Genotipados) %>%
  summarise(Correlación = round(cor(Predicho, Observado), digits =  2)) %>%
  ggplot(data = ., aes(x = Densidad, y = Correlación, fill = Genotipados, colour = Genotipados, label = Correlación)) +
  geom_col(position = position_dodge(width = 0.7), width = 0.6, alpha = 0.4) +
  ...
  scale_fill_manual(values = c('yellow', 'cyan', 'red', 'black')) +
  scale_y_continuous(limits = c(0.0, 1.0)) +
  labs(y = 'Precisión de la predicción (r)', x = 'Densidad del marcador', title = 'Pedigrí 2') +
  theme_bw() +
  theme(
    axis.text = element_text(size = 14),
    axis.title.y = element_text(size = 12, face = 'bold'),
...
Cor_ped1_ped2 <- Cor_ped1 + Cor_ped2 & theme(legend.position = 'bottom')
Cor_ped1_ped2 + plot_layout(guides = 'collect')

# Pedigrí 3 ----

## Se calcula la correlación entre los fenotipos observados (y) y predichos (ebv) considerando una densidad de SNPs de 1.000 y todos los individuos genotipados:

Fen_pred_1 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_3', 'chip_1', 'Geno_1', 'ebv_1.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_1 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_3', 'chip_1', 'Geno_1', 'Fenotipo_1.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_1_ped3 <- bind_cols(Fen_pred_1, Fen_obse_1) %>%
  slice(1263:2062) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1262 al 2061), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '1.000', Genotipados = 'F0-F1-F2-F3')
...
Fen_pred_2 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_3', 'chip_1', 'Geno_2', 'ebv_2.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_2 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_3', 'chip_1', 'Geno_2', 'Fenotipo_2.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_2_ped3 <- bind_cols(Fen_pred_2, Fen_obse_2) %>%
  slice(1263:2062) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1262 al 2061), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '1.000', Genotipados = 'F1-F2')
...
Fen_pred_3 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_3', 'chip_1', 'Geno_3', 'ebv_3.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_3 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_3', 'chip_1', 'Geno_3', 'Fenotipo_3.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_3_ped3 <- bind_cols(Fen_pred_3, Fen_obse_3) %>%
  slice(1263:2062) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1262 al 2061), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '1.000', Genotipados = 'F2')
...
Fen_pred_4 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_3', 'chip_2', 'Geno_1', 'ebv_1.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_4 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_3', 'chip_2', 'Geno_1', 'Fenotipo_1.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_4_ped3 <- bind_cols(Fen_pred_4, Fen_obse_4) %>%
  slice(1263:2062) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1262 al 2061), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '10.000', Genotipados = 'F0-F1-F2-F3')
...
Fen_pred_5 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_3', 'chip_2', 'Geno_2', 'ebv_2.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_5 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_3', 'chip_2', 'Geno_2', 'Fenotipo_2.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_5_ped3 <- bind_cols(Fen_pred_5, Fen_obse_5) %>%
  slice(1263:2062) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1262 al 2061), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '10.000', Genotipados = 'F1-F2')
...
Fen_pred_6 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_3', 'chip_2', 'Geno_3', 'ebv_3.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_6 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_3', 'chip_2', 'Geno_3', 'Fenotipo_3.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_6_ped3 <- bind_cols(Fen_pred_6, Fen_obse_6) %>%
  slice(1263:2062) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1262 al 2061), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '10.000', Genotipados = 'F2')
...
Fen_pred_7 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_3', 'chip_3', 'Geno_1', 'ebv_1.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_7 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_3', 'chip_3', 'Geno_1', 'Fenotipo_1.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_7_ped3 <- bind_cols(Fen_pred_7, Fen_obse_7) %>%
  slice(1263:2062) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1262 al 2061), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '100.000', Genotipados = 'F0-F1-F2-F3')
...
Fen_pred_8 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_3', 'chip_3', 'Geno_2', 'ebv_2.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_8 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_3', 'chip_3', 'Geno_2', 'Fenotipo_2.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_8_ped3 <- bind_cols(Fen_pred_8, Fen_obse_8) %>%
  slice(1263:2062) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1262 al 2061), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '100.000', Genotipados = 'F1-F2')
...
Fen_pred_9 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_3', 'chip_3', 'Geno_3', 'ebv_3.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_9 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_3', 'chip_3', 'Geno_3', 'Fenotipo_3.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_9_ped3 <- bind_cols(Fen_pred_9, Fen_obse_9) %>%
  slice(1263:2062) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1262 al 2061), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '100.000', Genotipados = 'F2')
...
Fen_pred_10 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_3', 'chip_0', 'ebv.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_10 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_3', 'chip_0', 'Fenotipo.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_10_ped3 <- bind_cols(Fen_pred_10, Fen_obse_10) %>%
  slice(1263:2062) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1262 al 2061), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '0', Genotipados = 'Ninguno')

Cor_ped3 <- bind_rows(Cor_1_ped3, Cor_2_ped3, Cor_3_ped3, Cor_4_ped3, Cor_5_ped3, Cor_6_ped3, Cor_7_ped3, Cor_8_ped3, Cor_9_ped3, Cor_10_ped3) %>%
  group_by(Densidad, Genotipados) %>%
  summarise(Correlación = round(cor(Predicho, Observado), digits =  2)) %>%
  ggplot(data = ., aes(x = Densidad, y = Correlación, fill = Genotipados, colour = Genotipados, label = Correlación)) +
  geom_col(position = position_dodge(width = 0.7), width = 0.6, alpha = 0.4) +
  ...
  scale_fill_manual(values = c('yellow', 'cyan', 'red', 'black')) +
  scale_y_continuous(limits = c(0.0, 1.0)) +
  labs(y = 'Precisión de la predicción (r)', x = 'Densidad del marcador', title = 'Pedigrí 3') +
  theme_bw() +
  theme(
    axis.text = element_text(size = 14),
    axis.title.y = element_text(size = 12, face = 'bold'),
...
Fen_pred_1 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4', 'chip_1', 'Geno_1', 'ebv_1.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_1 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4', 'chip_1', 'Geno_1', 'Fenotipo_1.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_1_ped4 <- bind_cols(Fen_pred_1, Fen_obse_1) %>%
  slice(1333:2132) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1331 al 2131), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '1.000', Genotipados = 'F0-F1-F2-F3')
...
Fen_pred_2 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4', 'chip_1', 'Geno_2', 'ebv_2.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_2 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4', 'chip_1', 'Geno_2', 'Fenotipo_2.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_2_ped4 <- bind_cols(Fen_pred_2, Fen_obse_2) %>%
  slice(1333:2132) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1331 al 2131), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '1.000', Genotipados = 'F1-F2')
...
Fen_pred_3 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4', 'chip_1', 'Geno_3', 'ebv_3.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_3 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4', 'chip_1', 'Geno_3', 'Fenotipo_3.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_3_ped4 <- bind_cols(Fen_pred_3, Fen_obse_3) %>%
  slice(1333:2132) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1331 al 2131), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '1.000', Genotipados = 'F2')
...
Fen_pred_4 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4', 'chip_2', 'Geno_1', 'ebv_1.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_4 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4', 'chip_2', 'Geno_1', 'Fenotipo_1.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_4_ped4 <- bind_cols(Fen_pred_4, Fen_obse_4) %>%
  slice(1333:2132) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1331 al 2131), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '10.000', Genotipados = 'F0-F1-F2-F3')
...
Fen_pred_5 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4', 'chip_2', 'Geno_2', 'ebv_2.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_5 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4', 'chip_2', 'Geno_2', 'Fenotipo_2.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_5_ped4 <- bind_cols(Fen_pred_5, Fen_obse_5) %>%
  slice(1333:2132) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1331 al 2131), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '10.000', Genotipados = 'F1-F2')
...
Fen_pred_6 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4', 'chip_2', 'Geno_3', 'ebv_3.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_6 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4', 'chip_2', 'Geno_3', 'Fenotipo_3.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_6_ped4 <- bind_cols(Fen_pred_6, Fen_obse_6) %>%
  slice(1333:2132) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1331 al 2131), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '10.000', Genotipados = 'F2')
...
Fen_pred_7 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4', 'chip_3', 'Geno_1', 'ebv_1.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_7 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4', 'chip_3', 'Geno_1', 'Fenotipo_1.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_7_ped4 <- bind_cols(Fen_pred_7, Fen_obse_7) %>%
  slice(1333:2132) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1331 al 2131), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '100.000', Genotipados = 'F0-F1-F2-F3')
...
Fen_pred_8 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4', 'chip_3', 'Geno_2', 'ebv_2.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_8 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4', 'chip_3', 'Geno_2', 'Fenotipo_2.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_8_ped4 <- bind_cols(Fen_pred_8, Fen_obse_8) %>%
  slice(1333:2132) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1331 al 2131), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '100.000', Genotipados = 'F1-F2')
...
Fen_pred_9 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4', 'chip_3', 'Geno_3', 'ebv_3.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_9 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4', 'chip_3', 'Geno_3', 'Fenotipo_3.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_9_ped4 <- bind_cols(Fen_pred_9, Fen_obse_9) %>%
  slice(1333:2132) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1331 al 2131), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '100.000', Genotipados = 'F2')
...
Fen_pred_10 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4', 'chip_0', 'ebv.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_10 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4', 'chip_0', 'Fenotipo.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_10_ped4 <- bind_cols(Fen_pred_10, Fen_obse_10) %>%
  slice(1333:2132) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1331 al 2131), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '0', Genotipados = 'Ninguno')

Cor_ped4 <- bind_rows(Cor_1_ped4, Cor_2_ped4, Cor_3_ped4, Cor_4_ped4, Cor_5_ped4, Cor_6_ped4, Cor_7_ped4, Cor_8_ped4, Cor_9_ped4, Cor_10_ped4) %>%
  group_by(Densidad, Genotipados) %>%
  summarise(Correlación = round(cor(Predicho, Observado), digits =  2)) %>%
  ggplot(data = ., aes(x = Densidad, y = Correlación, fill = Genotipados, colour = Genotipados, label = Correlación)) +
  geom_col(position = position_dodge(width = 0.7), width = 0.6, alpha = 0.4) +
  ...
  scale_fill_manual(values = c('yellow', 'cyan', 'red', 'black')) +
  scale_y_continuous(limits = c(0.0, 1.0)) +
  labs(y = 'Precisión de la predicción (r)', x = 'Densidad del marcador', title = 'Pedigrí 4') +
  theme_bw() +
  theme(
    axis.text = element_text(size = 14),
    axis.title.y = element_text(size = 12, face = 'bold'),
...
Cor_ped3_ped4 <- Cor_ped3 + Cor_ped4 & theme(legend.position = 'bottom')
Cor_ped3_ped4 + plot_layout(guides = 'collect')

# Pedigrí 1, 2, 3 y 4

Cor_ped1_ped2_ped3_ped4 <- Cor_ped1 + Cor_ped2 + Cor_ped3 + Cor_ped4 & theme(legend.position = 'right')
Cor_ped1_ped2_ped3_ped4 + plot_layout(guides = 'collect')
```

<!--
19. Se obtienen los resultados para el pedigrí 4, esta vez quitando también el fenotipo a los individuos de la generación F2 como una prueba de que la predicción esta bien.
-->

```{python Prueba con el pedigrí 4 considerando la generación F2 y F3 sin fenotipo}
...
os.mkdir('Pedigri_4b')
os.mkdir('Pedigri_4b/chip_1') # Se crea la carpeta que contendra todos los resultados al realizar ssGBLUP con una densidad de 1.000 SNPs.
...
os.mkdir('Pedigri_4b/chip_1/Geno_1') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con todos los individuos genotipados.
...
ind_fen = np.arange(531, dtype = np.int) # Aquí se crea el objeto "ind_fen" que indica que individuos tendrían medida fenotípica (del individuo 1 al 1331, esto es, ningún individuo de la generación F3 tiene fenotipo).
...
ebv_ssGBLUP_1 = np.array(list(Pob_4.inds[i].ebv for i in range(Pob_4.n)))
ebv_ssGBLUP_1 = pd.DataFrame(ebv_ssGBLUP_1)
ebv_ssGBLUP_1.to_csv (r'Pedigri_4b/chip_1/Geno_1/ebv_1.csv', index = False, header = True)

y_1 = np.array(list(Pob_4.inds[i].y[itrait] for i in range(Pob_4.n)))
y_1 = pd.DataFrame(y_1)
y_1.to_csv (r'Pedigri_4b/chip_1/Geno_1/Fenotipo_1.csv', index = False, header = True)

g_add_1 = np.array(list(Pob_4.inds[i].g_add[itrait] for i in range(Pob_4.n)))
g_add_1 = pd.DataFrame(g_add_1)
g_add_1.to_csv (r'Pedigri_4b/chip_1/Geno_1/g_add_1.csv', index = False, header = True)
...
os.mkdir('Pedigri_4b/chip_1/Geno_2') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.
...
ebv_ssGBLUP_2 = np.array(list(Pob_4.inds[i].ebv for i in range(Pob_4.n)))
ebv_ssGBLUP_2 = pd.DataFrame(ebv_ssGBLUP_2)
ebv_ssGBLUP_2.to_csv (r'Pedigri_4b/chip_1/Geno_2/ebv_2.csv', index = False, header = True)

y_2 = np.array(list(Pob_4.inds[i].y[itrait] for i in range(Pob_4.n)))
y_2 = pd.DataFrame(y_2)
y_2.to_csv (r'Pedigri_4b/chip_1/Geno_2/Fenotipo_2.csv', index = False, header = True)

g_add_2 = np.array(list(Pob_4.inds[i].g_add[itrait] for i in range(Pob_4.n)))
g_add_2 = pd.DataFrame(g_add_2)
g_add_2.to_csv (r'Pedigri_4b/chip_1/Geno_2/g_add_2.csv', index = False, header = True)
...
os.mkdir('Pedigri_4b/chip_1/Geno_3') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.
...
ebv_ssGBLUP_3 = np.array(list(Pob_4.inds[i].ebv for i in range(Pob_4.n)))
ebv_ssGBLUP_3 = pd.DataFrame(ebv_ssGBLUP_3)
ebv_ssGBLUP_3.to_csv (r'Pedigri_4b/chip_1/Geno_3/ebv_3.csv', index = False, header = True)

y_3 = np.array(list(Pob_4.inds[i].y[itrait] for i in range(Pob_4.n)))
y_3 = pd.DataFrame(y_3)
y_3.to_csv (r'Pedigri_4b/chip_1/Geno_3/Fenotipo_3.csv', index = False, header = True)

g_add_3 = np.array(list(Pob_4.inds[i].g_add[itrait] for i in range(Pob_4.n)))
g_add_3 = pd.DataFrame(g_add_3)
g_add_3.to_csv (r'Pedigri_4b/chip_1/Geno_3/g_add_3.csv', index = False, header = True)

############################
# Densidad de SNPs de 10.000
############################

os.mkdir('Pedigri_4b/chip_2') # Se crea la carpeta que contendra todos los resultados al realizar ssGBLUP con una densidad de 10.000 SNPs.
...
os.mkdir('Pedigri_4b/chip_2/Geno_1') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con todos los individuos genotipados.
...
ebv_ssGBLUP_1 = np.array(list(Pob_4.inds[i].ebv for i in range(Pob_4.n)))
ebv_ssGBLUP_1 = pd.DataFrame(ebv_ssGBLUP_1)
ebv_ssGBLUP_1.to_csv (r'Pedigri_4b/chip_2/Geno_1/ebv_1.csv', index = False, header = True)

y_1 = np.array(list(Pob_4.inds[i].y[itrait] for i in range(Pob_4.n)))
y_1 = pd.DataFrame(y_1)
y_1.to_csv (r'Pedigri_4b/chip_2/Geno_1/Fenotipo_1.csv', index = False, header = True)

g_add_1 = np.array(list(Pob_4.inds[i].g_add[itrait] for i in range(Pob_4.n)))
g_add_1 = pd.DataFrame(g_add_1)
g_add_1.to_csv (r'Pedigri_4b/chip_2/Geno_1/g_add_1.csv', index = False, header = True)
...
os.mkdir('Pedigri_4b/chip_2/Geno_2') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.
...
ebv_ssGBLUP_2 = np.array(list(Pob_4.inds[i].ebv for i in range(Pob_4.n)))
ebv_ssGBLUP_2 = pd.DataFrame(ebv_ssGBLUP_2)
ebv_ssGBLUP_2.to_csv (r'Pedigri_4b/chip_2/Geno_2/ebv_2.csv', index = False, header = True)

y_2 = np.array(list(Pob_4.inds[i].y[itrait] for i in range(Pob_4.n)))
y_2 = pd.DataFrame(y_2)
y_2.to_csv (r'Pedigri_4b/chip_2/Geno_2/Fenotipo_2.csv', index = False, header = True)

g_add_2 = np.array(list(Pob_4.inds[i].g_add[itrait] for i in range(Pob_4.n)))
g_add_2 = pd.DataFrame(g_add_2)
g_add_2.to_csv (r'Pedigri_4b/chip_2/Geno_2/g_add_2.csv', index = False, header = True)
...
os.mkdir('Pedigri_4b/chip_2/Geno_3') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.
...
ebv_ssGBLUP_3 = np.array(list(Pob_4.inds[i].ebv for i in range(Pob_4.n)))
ebv_ssGBLUP_3 = pd.DataFrame(ebv_ssGBLUP_3)
ebv_ssGBLUP_3.to_csv (r'Pedigri_4b/chip_2/Geno_3/ebv_3.csv', index = False, header = True)

y_3 = np.array(list(Pob_4.inds[i].y[itrait] for i in range(Pob_4.n)))
y_3 = pd.DataFrame(y_3)
y_3.to_csv (r'Pedigri_4b/chip_2/Geno_3/Fenotipo_3.csv', index = False, header = True)

g_add_3 = np.array(list(Pob_4.inds[i].g_add[itrait] for i in range(Pob_4.n)))
g_add_3 = pd.DataFrame(g_add_3)
g_add_3.to_csv (r'Pedigri_4b/chip_2/Geno_3/g_add_3.csv', index = False, header = True)

#############################
# Densidad de SNPs de 100.000
#############################

os.mkdir('Pedigri_4b/chip_3') # Se crea la carpeta que contendra todos los resultados al realizar ssGBLUP con una densidad de 100.000 SNPs.
...
os.mkdir('Pedigri_4b/chip_3/Geno_1') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con todos los individuos genotipados.
...
ebv_ssGBLUP_1 = np.array(list(Pob_4.inds[i].ebv for i in range(Pob_4.n)))
ebv_ssGBLUP_1 = pd.DataFrame(ebv_ssGBLUP_1)
ebv_ssGBLUP_1.to_csv (r'Pedigri_4b/chip_3/Geno_1/ebv_1.csv', index = False, header = True)

y_1 = np.array(list(Pob_4.inds[i].y[itrait] for i in range(Pob_4.n)))
y_1 = pd.DataFrame(y_1)
y_1.to_csv (r'Pedigri_4b/chip_3/Geno_1/Fenotipo_1.csv', index = False, header = True)

g_add_1 = np.array(list(Pob_4.inds[i].g_add[itrait] for i in range(Pob_4.n)))
g_add_1 = pd.DataFrame(g_add_1)
g_add_1.to_csv (r'Pedigri_4b/chip_3/Geno_1/g_add_1.csv', index = False, header = True)
...
os.mkdir('Pedigri_4b/chip_3/Geno_2') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.
...
ebv_ssGBLUP_2 = np.array(list(Pob_4.inds[i].ebv for i in range(Pob_4.n)))
ebv_ssGBLUP_2 = pd.DataFrame(ebv_ssGBLUP_2)
ebv_ssGBLUP_2.to_csv (r'Pedigri_4b/chip_3/Geno_2/ebv_2.csv', index = False, header = True)

y_2 = np.array(list(Pob_4.inds[i].y[itrait] for i in range(Pob_4.n)))
y_2 = pd.DataFrame(y_2)
y_2.to_csv (r'Pedigri_4b/chip_3/Geno_2/Fenotipo_2.csv', index = False, header = True)

g_add_2 = np.array(list(Pob_4.inds[i].g_add[itrait] for i in range(Pob_4.n)))
g_add_2 = pd.DataFrame(g_add_2)
g_add_2.to_csv (r'Pedigri_4b/chip_3/Geno_2/g_add_2.csv', index = False, header = True)
...
os.mkdir('Pedigri_4b/chip_3/Geno_3') # Se crea la carpeta que contendra los resultados al realizar ssGBLUP con los individuos de la generación F2 genotipados.
...
ebv_ssGBLUP_3 = np.array(list(Pob_4.inds[i].ebv for i in range(Pob_4.n)))
ebv_ssGBLUP_3 = pd.DataFrame(ebv_ssGBLUP_3)
ebv_ssGBLUP_3.to_csv (r'Pedigri_4b/chip_3/Geno_3/ebv_3.csv', index = False, header = True)

y_3 = np.array(list(Pob_4.inds[i].y[itrait] for i in range(Pob_4.n)))
y_3 = pd.DataFrame(y_3)
y_3.to_csv (r'Pedigri_4b/chip_3/Geno_3/Fenotipo_3.csv', index = False, header = True)

g_add_3 = np.array(list(Pob_4.inds[i].g_add[itrait] for i in range(Pob_4.n)))
g_add_3 = pd.DataFrame(g_add_3)
g_add_3.to_csv (r'Pedigri_4b/chip_3/Geno_3/g_add_3.csv', index = False, header = True)
```
...
os.mkdir('Pedigri_4b/chip_0') # Se crea la carpeta que contendra los resultados al realizar aBLUP.

## 1. Se carga el objeto "Pob_4":
...
ebv_aBLUP = np.array(list(Pob_4.inds[i].ebv for i in range(Pob_4.n)))
ebv_aBLUP = pd.DataFrame(ebv_aBLUP)
ebv_aBLUP.to_csv (r'Pedigri_4b/chip_0/ebv.csv', index = False, header = True)

y_aBLUP = np.array(list(Pob_4.inds[i].y[itrait] for i in range(Pob_4.n)))
y_aBLUP = pd.DataFrame(y_aBLUP)
y_aBLUP.to_csv (r'Pedigri_4b/chip_0/Fenotipo.csv', index = False, header = True)

g_add = np.array(list(Pob_4.inds[i].g_add[itrait] for i in range(Pob_4.n)))
g_add = pd.DataFrame(g_add)
g_add.to_csv (r'Pedigri_4b/chip_0/g_add.csv', index = False, header = True)
```
...
Fen_pred_1 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4b', 'chip_1', 'Geno_1', 'ebv_1.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_1 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4b', 'chip_1', 'Geno_1', 'Fenotipo_1.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_1_ped4b <- bind_cols(Fen_pred_1, Fen_obse_1) %>%
  slice(533:2132) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 531 al 2131), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '1.000', Genotipados = 'Todos')

## Se calcula la correlación entre los fenotipos observados (y) y predichos (ebv) considerando una densidad de SNPs de 1.000 y los individuos de las generaciones F1 y F2 genotipados:

Fen_pred_2 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4b', 'chip_1', 'Geno_2', 'ebv_2.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_2 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4b', 'chip_1', 'Geno_2', 'Fenotipo_2.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_2_ped4b <- bind_cols(Fen_pred_2, Fen_obse_2) %>%
  slice(533:2132) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 531 al 2131), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '1.000', Genotipados = 'F1 y F2')

## Se calcula la correlación entre los fenotipos observados (y) y predichos (ebv) considerando una densidad de SNPs de 1.000 y los individuos de la generación F2 genotipados:

Fen_pred_3 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4b', 'chip_1', 'Geno_3', 'ebv_3.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_3 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4b', 'chip_1', 'Geno_3', 'Fenotipo_3.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_3_ped4b <- bind_cols(Fen_pred_3, Fen_obse_3) %>%
  slice(533:2132) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 531 al 2131), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '1.000', Genotipados = 'F2')
...
Fen_pred_4 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4b', 'chip_2', 'Geno_1', 'ebv_1.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_4 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4b', 'chip_2', 'Geno_1', 'Fenotipo_1.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_4_ped4b <- bind_cols(Fen_pred_4, Fen_obse_4) %>%
  slice(533:2132) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 531 al 2131), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '10.000', Genotipados = 'Todos')

## Se calcula la correlación entre los fenotipos observados (y) y predichos (ebv) considerando una densidad de SNPs de 10.000 y los individuos de la generación F1 y F2 genotipados:

Fen_pred_5 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4b', 'chip_2', 'Geno_2', 'ebv_2.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_5 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4b', 'chip_2', 'Geno_2', 'Fenotipo_2.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_5_ped4b <- bind_cols(Fen_pred_5, Fen_obse_5) %>%
  slice(533:2132) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 531 al 2131), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '10.000', Genotipados = 'F1 y F2')

## Se calcula la correlación entre los fenotipos observados (y) y predichos (ebv) considerando una densidad de SNPs de 10.000 y los individuos de la generación F2 genotipados:

Fen_pred_6 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4b', 'chip_2', 'Geno_3', 'ebv_3.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_6 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4b', 'chip_2', 'Geno_3', 'Fenotipo_3.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_6_ped4b <- bind_cols(Fen_pred_6, Fen_obse_6) %>%
  slice(533:2132) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 531 al 2131), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '10.000', Genotipados = 'F2')
...
Fen_pred_7 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4b', 'chip_3', 'Geno_1', 'ebv_1.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_7 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4b', 'chip_3', 'Geno_1', 'Fenotipo_1.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_7_ped4b <- bind_cols(Fen_pred_7, Fen_obse_7) %>%
  slice(533:2132) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 531 al 2131), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '100.000', Genotipados = 'Todos')

## Se calcula la correlación entre los fenotipos observados (y) y predichos (ebv) considerando una densidad de SNPs de 100.000 y los individuos de las generaciones F1 y F2 genotipados:

Fen_pred_8 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4b', 'chip_3', 'Geno_2', 'ebv_2.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_8 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4b', 'chip_3', 'Geno_2', 'Fenotipo_2.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_8_ped4b <- bind_cols(Fen_pred_8, Fen_obse_8) %>%
  slice(533:2132) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 531 al 2131), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '100.000', Genotipados = 'F1 y F2')

## Se calcula la correlación entre los fenotipos observados (y) y predichos (ebv) considerando una densidad de SNPs de 100.000 y los individuos de la generaciónes F2 genotipados:

Fen_pred_9 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4b', 'chip_3', 'Geno_3', 'ebv_3.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_9 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4b', 'chip_3', 'Geno_3', 'Fenotipo_3.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_9_ped4b <- bind_cols(Fen_pred_9, Fen_obse_9) %>%
  slice(533:2132) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 531 al 2131), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '100.000', Genotipados = 'F2')
...
Fen_pred_10 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4b', 'chip_0', 'ebv.csv'), delim = ' ', col_names = FALSE) # Se importan los fenotipos predichos.

Fen_obse_10 <- read_delim(here('Datos', 'Datos_IND', 'SeqBreed', 'Pedigri_4', 'chip_0', 'Fenotipo.csv'), delim = ' ', col_names = FALSE) # Se importa los fenotipos observados.

Cor_10_ped4b <- bind_cols(Fen_pred_10, Fen_obse_10) %>%
  slice(533:2132) %>% # Se deja aquí solo los individuos de la generación F3 (del individuo 1331 al 2131), ya que en base a ellos se calcula la habilidad predictiva.
  #summarise(cor(X1...1, X1...2)) %>% # Calculo de la correlación.
  rename(Predicho = X1...1, Observado = X1...2) %>%
  mutate(Densidad = '0', Genotipados = 'Ninguno')

Cor_ped4b <- bind_rows(Cor_1_ped4b, Cor_2_ped4b, Cor_3_ped4b, Cor_4_ped4b, Cor_5_ped4b, Cor_6_ped4b, Cor_7_ped4b, Cor_8_ped4b, Cor_9_ped4b, Cor_10_ped4b) %>%
  group_by(Densidad, Genotipados) %>%
  summarise(Correlación = round(cor(Predicho, Observado), digits =  2)) %>%
  ggplot(data = ., aes(x = Densidad, y = Correlación, fill = Genotipados, colour = Genotipados, label = Correlación)) +
  geom_col(position = position_dodge(width = 0.7), width = 0.6, alpha = 0.4) +
  geom_label(colour = 'black', fill = 'white', size = 4, position = position_dodge2(width = 1.0), vjust=-0.3) +
  scale_colour_manual(values = c('yellow', 'cyan', 'black', 'red')) +
  scale_fill_manual(values = c('yellow', 'cyan', 'black', 'red')) +
  scale_y_continuous(limits = c(0.0, 1.0)) +
  labs(y = 'Precisión de la predicción (r)', x = 'Densidad del marcador', title = 'Pedigrí 4 (prueba)') +
  theme_bw() +
  theme(
    axis.text = element_text(size = 14),
    axis.title = element_text(size = 15, face = 'bold'),
